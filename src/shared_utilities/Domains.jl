...
classified as inland water, since the inland water soil overrides
are not appropriate for ice sheets.
...
function inland_water_mask(
    ...
    # Exclude polar regions where the inland water overrides are inappropriate
    ...
    if hasproperty(ClimaCore.Fields.coordinate_field(surface_space), :lat)
        ...
        in_bounds = within_latitude_bounds.(lat_field, lat_min, lat_max)
...
within_latitude_bounds(lat, lat_min, lat_max) =
...
Helper function to create a `SphericalShell` domain with `nelements` elements, a
...
case, the resolution of the model is sqrt((360 x 180)/(101 x 101 x 6)). The factor of 6 arises
because there are 101x101 elements per side of the cubed sphere, meaning 6 x 101 x 101 for the
entire globe.
...
function global_box_domain(
    ...
    radius_earth = FT(6.378e6)
    zlim = (-FT(depth), FT(0.0))
    xlim = FT.((π * radius_earth, π * radius_earth))
