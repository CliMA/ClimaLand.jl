...
β\\_scf = max(β0 - γ(horz\\_degree\\_res - 1.5), β\\_min), where horz\\_degree\\_res is the
horizontal resolution of the simulation, in degrees, and β0, β\\_min and γ
are unitless. It is correct to think of β0, β\\_min, γ, and z0 as the free
parameters, while horz_degree_res is provided and β_scf is determined.

β0, β_min, γ, and β\\_scf must be > 0.
