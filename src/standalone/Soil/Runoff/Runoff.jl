...
function ClimaLand.source!(
    ...
    h∇ = p.soil.h∇
    ϵ = eps(FT)
    @. dY.soil.ϑ_l -= (Y.soil.ϑ_l - model.parameters.ν) * p.soil.is_saturated/FT(2.6e5) #(p.soil.R_ss / max(h∇, ϵ)) * p.soil.is_saturated # apply only to saturated layers
    @. dY.soil.ρe_int -= (Y.soil.ϑ_l - model.parameters.ν) * p.soil.is_saturated/FT(2.6e5) * volumetric_internal_energy_liq(p.soil.T, model.parameters.earth_param_set)##p.so
#   @. dY.soil.ϑ_l -= (p.soil.R_ss / max(h∇, ϵ)) * p.soil.is_saturated # apply only to saturated layers
