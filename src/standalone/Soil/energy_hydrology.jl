...
function ClimaLSM.add_drivers_to_cache(p, model::EnergyHydrology{FT}) where {FT}
    bc = model.soil.boundary_conditions.top
    if typeof(bc) <: AtmosDrivenFluxBC{<:PrescribedAtmos, <: AbstractRadiativeDrivers, <:AbstractRunoffModel}
        vars = ClimaLSM.driver_p(bc.atmos, bc.radiation, ClimaLSM.Domains.coordinates(model))
        return merge(p, vars)
    else
        return p
    end
