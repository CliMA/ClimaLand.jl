...
function make_update_boundary_fluxes(model::EnergyHydrology)
    NVTX.@annotate "energy hyd fluxes" function update_boundary_fluxes!(p, Y, t)
...
function ClimaLand.make_compute_exp_tendency(
    ...
    NVTX.@annotate "enhyd exp tend" function compute_exp_tendency!(dY, Y, p, t)
...
function ClimaLand.make_compute_imp_tendency(
    ...
    NVTX.@annotate "enhyd imp tend" function compute_imp_tendency!(dY, Y, p, t)
        z = model.domain.fields.z
...
function ClimaLand.make_compute_jacobian(model::EnergyHydrology{FT}) where {FT}
    NVTX.@annotate "enhyd jacobian" function compute_jacobian!(
...
function ClimaLand.make_update_aux(model::EnergyHydrology)
    NVTX.@annotate "update_aux energyhyrdo" function update_aux!(p, Y, t)
        (;
            ν,
            hydrology_cm,
            K_sat,
