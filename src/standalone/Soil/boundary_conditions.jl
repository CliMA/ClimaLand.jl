...
function soil_boundary_fluxes!(
    ...
    @. p.soil.dfluxBCdY.heat = Geometry.Covariant3Vector(zero(eltype(eltype(p.soil.dfluxBCdY.heat))))  # replace 0 with ∂F∂T when ready
    @. p.soil.dfluxBCdY.water = Geometry.Covariant3Vector(zero(eltype(eltype(p.soil.dfluxBCdY.water))))
    return nothing
end
