...
Compute the optimal ratio of intercellular to ambient CO2 (chi = ci/ca) for C3 plants using P-model.
"""
...
Compute the optimal ratio of intercellular to ambient CO2 (chi = ci/ca) for C4 plants.
...
# Example values
```julia
Ao_daily = 0.4      # mol m^-2 day^-1 (summer day in temperate forest)
m = 7.0             # dimensionless (from compute_m)
k = 0.5             # dimensionless
LAI_max = 3.0       # m^2 m^-2

L_steady = compute_steady_state_LAI(Ao_daily, m, k, LAI_max)
# Returns: ~1.4 m^2 m^-2 (close to LAI_max during peak growing season)
```
...
# Example values
```julia
LAI_prev = 3.5      # m^2 m^-2 (LAI from yesterday)
L_steady = 4.0      # m^2 m^-2 (steady-state LAI for today)
...
# Returns: ~3.53 m^2 m^-2 (slowly approaching steady-state)
```
...
"""
    compute_PPFD(par_d::FT, lambda_PAR::FT, lightspeed::FT, planck_h::FT, N_a::FT) where {FT}
...
The function:
1. Computes instantaneous A0_daily using the P-model with fAPAR=1 and moisture stress factor beta
2. Computes instantaneous A0_annual using the P-model with fAPAR=1 and actual moisture stress factor beta
...
Both A0_daily and A0_annual use the actual moisture stress factor beta; water limitation also enters
LAI_max through the f0*P/A0 term using annual precipitation and VPD.
...
function call_update_optimal_LAI(
    ...
    # Compute instantaneous A0_daily with moisture stress factor beta (kg C m^-2 s^-1)
    A0_daily_inst = @. lazy(
        compute_A0_daily(
            is_c3,
            pmodel_parameters,
            ...
            ca,
            PPFD,
            βm,  # moisture stress factor beta
        ),
    )

    # Compute instantaneous A0_annual with actual moisture stress factor beta (kg C m^-2 s^-1)
    A0_annual_inst = @. lazy(
        compute_A0_daily(
            is_c3,
            pmodel_parameters,
    ...
    # Convert A0 from kg C m^-2 s^-1 to mol CO2 m^-2 per timestep
    ...
    # Accumulate to daily A0 (with moisture stress factor beta) - drives L_steady
    @. p.canopy.biomass.A0_daily_acc += A0_daily_per_timestep

    # Accumulate to daily A0_annual (with actual beta) - will be added to annual accumulator at noon
    @. p.canopy.biomass.A0_annual_daily_acc += A0_annual_per_timestep
    ...
    A0_annual_daily_final = @. p.canopy.biomass.A0_annual_daily_acc
    A0_annual_final = @. ifelse(
        local_noon_mask == FT(1) &&
        p.canopy.biomass.days_since_reset >= FT(365),
        p.canopy.biomass.A0_annual_acc,
        p.canopy.biomass.A0_annual,
    )

    # Compute ca_pa (CO2 partial pressure in Pa) and chi for water limitation term
    ...
    chi = @. lazy(
        compute_chi(
            ...
            T_canopy,
            P_air,
            p.canopy.biomass.vpd_gs,  # Use growing season mean VPD
            ca,
        ),
    )
    ...
    # Update A0_daily at noon (this stores the daily value with moisture stress factor beta)
    ...
    @. p.canopy.biomass.A0_annual_acc = ifelse(
        ...
        ifelse(
            local_noon_mask == FT(1),
            p.canopy.biomass.A0_annual_acc + A0_annual_daily_final,
            p.canopy.biomass.A0_annual_acc,
        ),
    )

    # Reset A0_daily_acc at noon
    @. p.canopy.biomass.A0_daily_acc = ifelse(
        local_noon_mask == FT(1),
        FT(0),
    ...
    @. p.canopy.biomass.A0_annual_daily_acc = ifelse(
        local_noon_mask == FT(1),
        FT(0),
        p.canopy.biomass.A0_annual_daily_acc,
    )
    ...
    @. p.canopy.biomass.days_since_reset = ifelse(
        ...
        ifelse(
            local_noon_mask == FT(1),
            p.canopy.biomass.days_since_reset + FT(1),
            p.canopy.biomass.days_since_reset,
        ),
    )
end
