...
where P is annual precipitation and A₀ is annual potential GPP.
...
- `A0_daily`: daily potential GPP from previous day (mol CO₂ m⁻² day⁻¹), with moisture stress factor β
- `A0_annual`: annual potential GPP from previous year (mol CO₂ m⁻² yr⁻¹), with actual moisture stress factor β
- `A0_daily_acc`: accumulator for current day's potential GPP with moisture stress factor β (mol CO₂ m⁻² day⁻¹)
- `A0_annual_acc`: accumulator for current year's potential GPP with actual moisture stress factor β (mol CO₂ m⁻² yr⁻¹)
- `A0_annual_daily_acc`: accumulator for current day's potential GPP with actual moisture stress factor β (mol CO₂ m⁻² day⁻¹)
...
# Arguments
- `Ao_annual::FT`: Annual total potential GPP (mol CO₂ m⁻² yr⁻¹).
...
- `A0_daily::FT`: Daily potential GPP (mol CO₂ m⁻² day⁻¹), with moisture stress factor β
...
- A0_daily uses moisture stress factor β to drive daily LAI dynamics
- A0_annual uses actual moisture stress factor β for LAI_max computation
...
The function:
1. Computes instantaneous A0_daily using the P-model with fAPAR=1 and moisture stress factor β
2. Computes instantaneous A0_annual using the P-model with fAPAR=1 and actual moisture stress factor β
...
Both A0_daily and A0_annual use the actual moisture stress factor β; water limitation also enters
...
function call_update_optimal_LAI(
    ...
    # Compute instantaneous A0_daily with moisture stress factor β (kg C m⁻² s⁻¹)
    A0_daily_inst = @. lazy(
        compute_A0_daily(
            is_c3,
            pmodel_parameters,
            ...
            ca,
            PPFD,
            βm,  # moisture stress factor β
        ),
    )

    # Compute instantaneous A0_annual with actual moisture stress factor β (kg C m⁻² s⁻¹)
    A0_annual_inst = @. lazy(
        compute_A0_daily(
            is_c3,
            pmodel_parameters,
            ...
            ca,
            PPFD,
            βm,  # use actual moisture stress factor
        ),
    )
    ...
    # Accumulate to daily A0 (with moisture stress factor β) - drives L_steady
    @. p.canopy.lai_model.A0_daily_acc += A0_daily_per_timestep

    # Accumulate to daily A0_annual (with actual β) - will be added to annual accumulator at noon
    ...
    # Update A0_daily at noon (this stores the daily value with moisture stress factor β)
    ...
    # Reset A0_annual_acc if year reset, otherwise add new daily A0_annual value at noon
...
Note: A0_daily_acc contains daily GPP with moisture stress factor β.
...
function update_A0_and_LAI_at_noon(
    ...
    if local_noon_mask == FT(1)
        ...
        # Finalize daily A0 (with moisture stress factor β for daily LAI dynamics)
        A0_daily = A0_daily_acc
        A0_daily_acc = FT(0)

        # Add today's A0_annual (for LAI_max) to annual accumulator
...
# Notes
- Daily A₀ is computed with fAPAR=1 and moisture stress factor β - drives L_steady
- Annual A₀ is computed with fAPAR=1 and actual moisture stress factor β - used for LAI_max
- Water limitation enters LAI_max through the f₀×P/A₀ × (cₐ(1-χ))/(1.6×D) term (Equation 11)
...
# Notes
- Daily A₀ is computed with moisture stress factor β - drives L_steady
- Annual A₀ is computed with actual moisture stress factor β - used for LAI_max
- Water limitation enters LAI_max through the f₀×P/A₀ × (cₐ(1-χ))/(1.6×D) term (Equation 11)
- GSL (Growing Season Length) is read from p.canopy.lai_model.GSL, which supports spatially
  varying values initialized via set_historical_cache! or optimal_lai_initial_conditions.
"""
