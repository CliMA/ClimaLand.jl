...
- `A0_daily_acc`: accumulator for current day's potential GPP with moisture stress factor beta (mol CO2 m^-2 day^-1)
- `A0_annual_acc`: accumulator for current year's potential GPP with actual moisture stress factor beta (mol CO2 m^-2 yr^-1)
- `A0_annual_daily_acc`: accumulator for current day's potential GPP with actual moisture stress factor beta (mol CO2 m^-2 day^-1)
...
# Notes
- precip_annual is converted from mm yr^-1 (input) to mol H2O m^-2 yr^-1 (internal units)
...
function set_historical_cache!(
    ...
    p.canopy.biomass.A0_annual_daily_acc .= FT(0)
