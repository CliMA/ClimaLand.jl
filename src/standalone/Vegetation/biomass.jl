...
function PrescribedAreaIndices(
    ...
    return PrescribedAreaIndices{FT, typeof(LAI)}(LAI, SAI, RAI)
end
...
function PrescribedBiomassModel{FT}(; LAI, SAI, RAI, rooting_depth) where {FT}
    ai_parameterization = PrescribedAreaIndices{FT}(LAI, SAI, RAI)
    args = (ai_parameterization, rooting_depth)
    return PrescribedBiomassModel{FT, typeof.(args)...}(args...)
end
...
function ClimaLand.Canopy.set_canopy_prescribed_field!(
    ...
    (; LAI, SAI, RAI) = component.ai_parameterization
...
"""
    make_biomass_callback(::PrescribedBiomassModel, canopy; start_date, Δt, update_period = Period(Day(8)))

Creates the callback which updates the LAI; this update is carried out every 8 days by default.
"""
...
"""
    get_model_callbacks(component::AbstractCanopyComponent, canopy; start_date, Δt)
...
function get_model_callbacks(
    ...
    lai_cb = make_biomass_callback(component, canopy; start_date, Δt)
    return (lai_cb,)
end
