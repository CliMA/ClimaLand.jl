...
"""
    OWUSConductanceModel{FT}(
        ...
        root_params,
        met_params = nothing,
        gsw_max = LP.get_default_parameter(FT, :owus_gsw_cap) # or FT(Inf)
    ) where {FT <: AbstractFloat}

Creates an OWUSConductanceModel using β(s) parameters diagnosed from
...
function OWUSConductanceModel{FT}(
    ...
    return OWUSConductanceModel{FT}(;
