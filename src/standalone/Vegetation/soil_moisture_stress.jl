...
- θ_high, θ_low should be in [0,ν] where ν is the porosity of the soil
...
function PiecewiseMoistureStressParameters(
    ...
    return PiecewiseMoistureStressParameters{FT}(θ_high, θ_low, c, β0, soil_zmin)
end
...
function PiecewiseMoistureStressParametersFromHydrology(
    ...
    verbose && println(
        "Constructing PiecewiseMoistureStressParameters: field capacity θ_high = $θ_high, wilting point θ_low = $θ_low",
    )
...
function update_soil_moisture_stress!(
    ...
    if :soil in canopy.boundary_conditions.prognostic_land_components
        ...
        # per soil element
        βm = @. lazy(compute_piecewise_moisture_stress(θ_high, θ_low, c, β0, ϑ_l))
