...
Base.@kwdef struct PiecewiseMoistureStressParameters{
    ...
    c::Union{FT, ClimaCore.Fields.Field}
    """Intrinsic moisture stress factor (unitless)"""
    β0::Union{FT, ClimaCore.Fields.Field}
    ...
    function PiecewiseMoistureStressParameters(
        ...
        if isa(soil_zmin, ClimaCore.Fields.Field)
            if any(>(0), soil_zmin)
                throw(ArgumentError("Soil zmin must be less than zero"))
            end
        else
            if soil_zmin > 0
                throw(ArgumentError("Soil zmin must be less than zero"))
        ...
        new{FT, F}(θ_c, θ_w, c, β0, soil_zmin)
    end
end
...
function update_soil_moisture_stress!(p, Y, model::PiecewiseMoistureStressModel, canopy)
    ...
    if :soil in canopy.boundary_conditions.prognostic_land_components 
        ...
        norm = @. lazy(Canopy.PlantHydraulics.root_distribution_CDF(
            soil_zmin,
        ...
        βm_root_distribution = @. lazy(βm * Canopy.PlantHydraulics.root_distribution(
            z, canopy.hydraulics.parameters.rooting_depth) / norm)
        
        # compute the root zone-averaged βm
        ClimaCore.Operators.column_integral_definite!(p.canopy.soil_moisture_stress.βm, βm_root_distribution)
    else
        (; θ_c, θ_w, c, β0, soil_zmin) = model.parameters
        @. p.canopy.soil_moisture_stress.βm = compute_piecewise_moisture_stress(
            θ_c, θ_w, c, β0, p.drivers.θ
        )
    end
end
