...
"""
    lake_energy_at_freezing(q_l::FT, params::SlabLakeParameters{FT}) where {FT}
...
function lake_energy_at_freezing(
    ...
    return _ρ_l * params.depth * cp * (_T_freeze - _T_ref) -
           _ρ_l * params.depth * (1 - q_l) * _LH_f0
end

"""
    lake_liquid_fraction(U::FT, params::SlabLakeParameters{FT}) where {FT}
...
"""
function lake_liquid_fraction(U::FT, params::SlabLakeParameters{FT}) where {FT}
    U_ice = lake_energy_at_freezing(FT(0), params)
    U_liq = lake_energy_at_freezing(FT(1), params)
    if U <= U_ice
        return FT(0)
    elseif U >= U_liq
        return FT(1)
...
"""
    lake_energy_from_temperature(T::FT, params::SlabLakeParameters{FT}) where {FT}
...
"""
    lake_temperature(U::FT, q_l::FT, params::SlabLakeParameters{FT}) where {FT}
