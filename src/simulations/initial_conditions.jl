...
function make_land_ic_file_from_output_diagnostics(inpath; infilestem = "_1M_average", outfilepath = joindir(inpath, "initial_conditions.nc"))
    attrib_swc = (;
    ...
    for i in 1:length(var_3d_attribs)
        ...
        data = NCDataset(joinpath(inpath, "$(varname)$(infilestem).nc"))
        z = data["z"][:]
        lat = data["lat"][:]
        lon = data["lon"][:]
        if i == 1
...
"""
    make_set_initial_state_from_previous_run(inpath, land::LandModel{FT}; enforce_constraints=false, infilestem = "_1M_average", outfilepath = joindir(inpath, "initial_conditions.nc")) where {FT}
"""
