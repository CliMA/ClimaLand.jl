...
# var_names, var_types, var_spaces a la ClimaAtmos
function initialize_vars(keys, types, state, model_name) # replace state with type and space info in future
    FT = eltype(state) # may not be needed
    # Code commented below was written with the intent of not using model_name, and make Y and p not be
    # hierarchical for standalone models.
    #=
    if length(keys) == 0
        return ClimaCore.Fields.FieldVector()
    else
        values = map((x) -> similar(state), keys) # the key line
        return ClimaCore.Fields.FieldVector(;
