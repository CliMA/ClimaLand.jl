...
Currently, this method will work for the following models:
- `CanopyModel`
- `SoilCanopyModel`
- `LandModel`
- `BucketModel`
`EnergyHydrology` has a specialized method that handles conservation diagnostics.
"""
function default_diagnostics(
    model::Union{
        CanopyModel{FT},
...
function default_diagnostics(
    ...
    soil_diagnostics = get_short_diagnostics(land_model)
    if average_period == :halfhourly
...
function default_diagnostics(
    ...
    @warn(
        "No default diagnostics defined for model type $(typeof(model)); consider extending `default_diagnostics` for this model type."
    )
    return []
end
...
function get_possible_diagnostics(model::EnergyHydrology{FT}) where {FT}
    return ["swc", "si", "sie", "tsoil", "et", "epa", "epac", "wvpa", "wvpac"]
end
