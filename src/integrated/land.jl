...
function make_exp_tendency(land::LandModel)
    ...
    NVTX.@annotate function exp_tendency!(dY, Y, p, t)
        ...
        if land.conservation
            ...
            @. dY.∫F_vol_liq_water_dt =  -( p.drivers.P_snow + p.drivers.P_liq + p.soil.R_s + p.snow.turbulent_fluxes.vapor_flux * p.snow.snow_cover_fraction + p.bare_soil_fraction * ( p.soil.turbulent_fluxes.vapor_flux_liq + p.soil.turbulent_fluxes.vapor_flux_ice) + p.canopy.turbulent_fluxes.vapor_flux - p.soil.bottom_bc.water + subsurface_runoff)
            @. dY.∫F_vol_liq_water_dt += lai_contribution
...
NVTX.@annotate function snow_boundary_fluxes!(
    ...
    @. p.snow.total_water_flux =
        ...
        p.snow.snow_cover_fraction
    residual_melt_flux = Snow.get_residual_surface_flux(
