...
that `LAI` is not a required argument and the default canopy model does not take an LAI as input.
Instead it uses `ZhouOptimalLAIModel` for the biomass component instead of `PrescribedBiomassModel`.
...
"""
    update_radiation_fluxes!(p, land::LandModel{FT},
...
"""
NVTX.@annotate function update_radiation_fluxes!(
    p,
    land::LandModel{FT},
    canopy_radiation::Canopy.AbstractRadiationModel{FT},
    Y,
    ...
    update_lake_lw_fluxes!(land.lake, p, _σ)
    LW_u_ground = @. lazy((p.bare_soil_fraction * LW_u_soil + p.snow.snow_cover_fraction * LW_u_snow)/(p.bare_soil_fraction + p.snow.snow_cover_fraction + eps(FT))
    @. LW_net_canopy =
    ...
    @. LW_u =
        ...
        p.snow.snow_cover_fraction * p.snow.LW_n + p.lake.LW_n * p.lake_fraction
...
function implicit_radiation_fluxes!(
    ...
    @. LW_u_soil = lw_ug(LW_d_canopy, ϵ_soil, T_soil, _σ)
    @. LW_u_snow = lw_ug(LW_d_canopy, ϵ_snow, T_snow, _σ)
    LW_u_ground = @. lazy((1-p.snow.snow_cover_fraction) * LW_u_soil + snow_frac * LW_u_snow)
    @. LW_net_canopy =
        ϵ_canopy * LW_d - 2 * ϵ_canopy * _σ * T_canopy^4 +
        ϵ_canopy * LW_u_ground
    @. LW_u =
...
"""
    update_lake_sw_fluxes!(lake::Nothing, p)
...
"""
    update_lake_lw_fluxes!(lake::Nothing, p, LW_u_lake, LW_d_canopy, _σ)
