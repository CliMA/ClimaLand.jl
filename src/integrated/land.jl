...
struct LandModel{
    FT,
    MM <: Union{Soil.Biogeochemistry.SoilCO2Model{FT}, NothingModel{FT}},
    SM <: Soil.EnergyHydrology{FT},
    VM <: Canopy.CanopyModel{FT},
    SnM <: Snow.SnowModel{FT},
} <: AbstractLandModel{FT}
    ...
    function LandModel{FT}(
        ...
        prognostic_land_components =
            soilco2 isa NothingModel ? (:canopy, :snow, :soil) :
        ...
        soilco2 isa NothingModel || @assert soil.parameters.earth_param_set ==
...
"""
    LandModel{FT}(
        ...
        soilco2 = prognostic_land_components == (:canopy, :snow, :soil, :soilco2) ?
...
ClimaLand.land_components(land::LandModel{FT, NothingModel{FT}}) where {FT} =
    (:soil, :snow, :canopy)
"""
