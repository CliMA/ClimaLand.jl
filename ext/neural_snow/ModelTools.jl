...
"""
#technically, this isn't the most-optimized version for speed, the speediest version would look like:
#    
#    result = Matrix{T}(undef, 3, length(pred))
#    # previous NN layers will ensure (otherwise they will throw
#    # an error prior to this function being called) `pred` and
#    # `input` are the right sizes to allow usage of @simd, @inbounds:
#    @inbounds @simd for i in eachindex(pred)
#        #Upper bound function:
#        result[1, i] = upperb(pred, input, i)
#        #Lower bound function:
#        result[2, i] = lowerb(pred, input, i)
#        #The predicted value:
#        result[3, i] = @inbounds pred[i]
#    end
#    return result
#
# and the upper and lower boundary functions would be defined per-index "i". However, this version
# cannot be autodifferentiated by Zygote within Flux, so models using these forms cannot be trained.
# However, I am amenable to including both forms and doing something like `trainable = true/false` as
# a keyword flag in the constructor to specify which one to use - the above form uses about half the allocations the
# below one does (original model used 60 pool allocations, below uses 29, and the above only needs 16.
# Both the above and below are faster than the old model by about a factor of 1.5, though 
# the above is faster than the below by about 10%, it just can't be trained):
...
# after you see/review this and the above about speed, I could also just get rid of this function entirely,
# since it's easy to put the one-line "connectfunc = [upperb(pred, input); lowerb(pred, input); pred]"
# directly into make_model() instead of calling this function, reducing steps. However, keeping this does
# also abstract/break-up function definitions to make it more understandable, so i'll leave that up to you
# whether this stays or not, if we don't care about implementing the fast-to-execute version and the
# able-to-train version separately. Getting rid of this does mean a user would have to edit make_model()
# instead of this function if they wanted to use boundary functions that took more inputs than just "pred, input".
...
- `input::AbstractArray{<:AbstractFloat}`: The array of inputs sent to the predictive component for each prediction.
    ...
    number of predictions.
"""
#the optimized version for execution-runtime (with indices) that cannot train:
#paper_upper_bound(
#    precip_idx::Int,
#    pred::AbstractArray{<:AbstractFloat},
#    input::AbstractArray{<:AbstractFloat},
#    i::Int
#    )::AbstractFloat = @inbounds (input[precip_idx, i] > 0) * relu(pred[i])
# additionally, in the version below (which is trainable), if assertions of function OUTPUT type are made,
# that somehow breaks the training capability of Flux? i.e. this functon as-is
# works, but if i did ")::AbstractArray{<:AbstractFloat}", it stops working?
paper_upper_bound(
    precip_idx::Int,
    pred::AbstractArray{<:AbstractFloat},
    input::AbstractArray{<:AbstractFloat},
...
- `input::AbstractArray{<:AbstractFloat}`: The array of inputs sent to the predictive component for each prediction.
    ...
    number of predictions.
"""
#the optimized version for runtime (with indices) that cannot train:
#paper_lower_bound(
#    z_i::Int,
#    pred::AbstractArray{<:AbstractFloat},
#    input::AbstractArray{<:AbstractFloat},
#    i::Int
#    )::AbstractFloat = @inbounds input[z_i, i]
# additionally, in the version below, if assertions of function OUTPUT type are made,
# that somehow breaks the training capability of Flux? i.e. this functon as-is
# works, but if i did ")::AbstractArray{<:AbstractFloat}", it stops working?
paper_lower_bound(
    depth_idx::Int,
...
function make_model(
    ...
    let nft = nfeatures,
        ...
        # if making your own, the skip-connection outputs [upper, lower, p]ᵀ    #<-- this line will change if we get rid of `connection_optimized()`
...
"""
#The new Flux training pipeline no longer uses parameters, this will be deleted
#if we lock the Flux version to be > 0.15, but have to keep this otherwise
#and dispatch the training function off the Flux version if we for some reason
# need to keep both:
function get_model_ps(model::Chain)
    return Flux.params(model[:pred].layers[2:4])
end
...
function trainmodel!(
    model,
    #ps, #this is now a useless argument - will get rid of after you see/review changes, now requires Flux version >= 0.15 though?
    x_train,
    y_train,
    n1,
    n2;
    ...
    # I dont like that Flux 0.15 now requires you to
    # state which weights you want to freeze, instead
    # of specifying which weights you want to train,
    # and requires the freeze to be carried within the `opt_state`
    # object instead of independently. This breaks the pipeline
    # and means this trainmodel!() function can only get used
    # for models made with `make_model()` and `make_model_paper()`,
    # ....unless it makes sense to abstract-out/break-up this function?
