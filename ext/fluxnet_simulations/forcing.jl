...
construction of PrescribedGroundConditions, which is done if `construct_prescribed_soil` is true.
...
If construct_prescribed_soil is true, then it also requires:
...
function FluxnetSimulations.prescribed_forcing_fluxnet(
    ...
    if construct_prescribed_soil && isnothing(soil_driver_args)
        error(
            "If construct_prescribed_soil is true, then soil_driver_args must be provided.
    ...
    varnames = get_required_varnames(construct_prescribed_soil)
    ...
    # soil temperature (ts) at the surface depth (1). TODO: consider reading in deeper depths, if 
    # it is available
    ...
    return construct_prescribed_soil ? (; atmos, radiation, soil) :
           (; atmos, radiation)
end
...
"""
    get_data_dates(
        site_ID,
        hour_offset_from_UTC;
        construct_prescribed_soil::Bool = false,
...
from UTC. Note that this will read the entire file to look for the start and 
end dates for which we have full data required to construct the drivers.

If `duration` is provided, it is used to determine the end date,
...
function FluxnetSimulations.get_data_dates(
    ...
    # get the columns where
    required_varnames = get_required_varnames(construct_prescribed_soil)
    column_name_map = create_column_name_map(columns, required_varnames)
    ...
    local_datetime =
        DateTime.(string.(Int.(data[valid_rows, 1])), "yyyymmddHHMM")
