...
This method is for a standalone canopy model.
"""
function FluxnetSimulations.make_set_fluxnet_initial_conditions(
    site_ID,
    start_date,
    ...
    set_ic!(Y, p, t, model) = set_fluxnet_ic!(Y, data, columns, Δ_date, model)
...
If the canopy energy model is a PrescribedCanopyTempModel, then the atmosphere air temperature
is used to set the canopy temperature.

If only a leaf compartment is used, only the leaf ψ is used.
"""
function set_fluxnet_ic!(
    Y,
    data,
    ...
    if !(model.energy isa Canopy.PrescribedCanopyTempModel)
