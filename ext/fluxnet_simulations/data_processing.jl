...
If `required_varnames` is provided, the data will first be filtered to only 
include rows where all required variables are available (not equal to `val`).
...
"""
    get_required_varnames(construct_soil_driver::Bool)

Returns the tuple of required variable names for Fluxnet data processing.
If `construct_soil_driver` is true, includes soil moisture and temperature variables.
"""
function get_required_varnames(construct_soil_driver::Bool)
    varnames = ("TA_F", "VPD_F", "PA_F", "P_F", "WS_F", "LW_IN_F", "SW_IN_F", "CO2_F_MDS")
...
Finds all rows where all required variables are available (not equal to the missing value flag).
Returns a vector of boolean values indicating which rows are valid.
"""
function find_rows_with_all_variables_available(data, column_name_map, varnames; val = -9999)
    valid_rows = Bool[]
    for i in 1:size(data, 1)
        row_valid = true
        for varname in varnames
            col_idx = column_name_map[varname]
            if data[i, col_idx] == val
                row_valid = false
                break
            end
        end
        push!(valid_rows, row_valid)
    end
    return valid_rows
end
