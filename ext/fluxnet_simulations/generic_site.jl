...
"""
    get_location(site_ID::String; kwargs)

Returns geographical and time information for a generic Fluxnet site.
The values are fetched from a list of Fluxnet sites metadata, and can
be overwritten by passing the corresponding keyword arguments to this
function.
"""
...
supplemented with additional autofilled values from global maps.
...
function FluxnetSimulations.get_parameters(
    ...
    ac_canopy = FT(pft.parameters.ac_canopy),
    ...
    h_leaf = ClimaCore.Fields.field2array(
        ClimaLand.Canopy.clm_canopy_height(domain.space.surface),
    )[1],
