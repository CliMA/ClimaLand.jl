...
function get_parameters(
    ...
    atmos_h = FT(2), # take max of each site, height >= h_canopy + 10
...
function get_pft(lat, long)
    ...
    dominant_pft = dominant_PFTs[pft_idx]
...
function get_soil_params_gupta(lat, long)
    soil_porosity_map_path = joinpath(ClimaLand.Artifacts.soil_params_artifact_folder_path(), 
        "porosity_map_gupta_etal2020_1.0x1.0x4.nc")
    soil_ksat_map_path = joinpath(ClimaLand.Artifacts.soil_params_artifact_folder_path(), 
        "ksat_map_gupta_etal2020_1.0x1.0x4.nc")
    soil_residual_map_path = joinpath(ClimaLand.Artifacts.soil_params_artifact_folder_path(), 
