...
"PA_F" (air pressure in ?) TODO: CHECK UNITS
"P_F" (accumulated precipitation in kg/m^2)
...
function prescribed_forcing_fluxnet(
    ...
    # precipitation is a downward flux (negative), and convert from a mass flux to a volume flux by dividing
    # by ρ_liq
    ...
    if split_precip
        ...
        atmos_P_liq = time_varying_input_from_data(
            ...
            column_name_map,
            seconds_since_start_date;
            preprocess_func = (x) -> -x / ρ_liq / data_dt,
        )
        atmos_P_snow = time_varying_input_from_data(
            data,
            "P_F",
