...
"""
function buildConstScaling(
    v::AbstractArray{FT},
)::ConstScaling{FT} where {FT <: AbstractFloat}
...
"""
function buildScaleOutput(x::FT)::ScaleOutput{FT} where {FT <: AbstractFloat}
    sc = [x]
...
function (model::ConstrainedNeuralModel{FT, C})(
    x,
) where {FT <: AbstractFloat, C <: ConstScaling{FT}} #typeof(x) & return-type left unspecified for faster training
...
function (model::ConstrainedNeuralModel{FT, C})(
    x,
) where {FT <: AbstractFloat, C <: NoScaling{FT}} #typeof(x) & return-type left unspecified for faster training
    return model.fixed_layers(
        boundary_connection(
            model.constraints,
