...
"""
struct NoScaling{FT} <: InputWeighting{FT} end
...
"""
function buildTwoSided(upper_bound, lower_bound)
...
Utility function for converting inputs to a ConstrainedNeuralModel to be of a consistent float type. Note that this function
does NOT work on the ConstrainedNeuralModel type itself, which has a fixed float-type. This is only a utility applied during the
construction of a ConstrainedNeuralModel.
"""
...
"""
function (model::ConstrainedNeuralModel{C})(
    x::AbstractArray{<:AbstractFloat},
)::AbstractArray{<:AbstractFloat} where {C <: ConstScaling}
...
"""
Flux.trainable(
    m::ConstrainedNeuralModel{S, Val{false}},
) where {S <: InputWeighting} = (; m.predictive_model)
...
"""
Flux.trainable(
    m::ConstrainedNeuralModel{S, Val{true}, C},
) where {S <: InputWeighting, C <: Union{<:UpperOnly, <:LowerOnly}} =
...
Flux.trainable(
    m::ConstrainedNeuralModel{S, Val{true}, C},
) where {S <: InputWeighting, C <: TwoSided} =
...
function buildConstraints(upper_bound, lower_bound)::ConstraintType
    ...
    elseif isnothing(upper_bound) && !isnothing(lower_bound)
        return buildLower(lower_bound)
    elseif !isnothing(upper_bound) && isnothing(lower_bound)
        return buildUpper(upper_bound)
    else
        return buildTwoSided(upper_bound, lower_bound)
    end
end
...
Returns the necessary scaling matrix to scale the output of a ConstrainedNeuralModel with a LowerOnly or UpperOnly constraint
...
"""
    get_scaling_matrix(constraint::TwoSided, use_out::FT)::Matrix{FT} where {FT <: AbstractFloat}

Returns the necessary scaling matrix to scale the output of a ConstrainedNeuralModel with a TwoSided constraint
...
"""
function load_model(strucure_filepath::String, param_filepath::String)
...
size, optmizer algorithm, or callback function, as well as to print training loss during the training process.
"""
function trainmodel!(
    model,
    x_train,
