...
"""
@bound_type struct Snow_Albedo_Bound{FT <: AbstractFloat}
    alb_idx::Int
    lim::FT
    t_scale_factor::Ref{FT}
end
...
(b.lim - input[b.alb_idx]) * b.t_scale_factor[] (b.t_scale_factor = 1/Δt)
"""
#need to test if this does not allocate
@bound function (b::Snow_Albedo_Bound)(
    pred::SVector{1, FT},
    input::SVector{6, FT},
)::FT where {FT <: AbstractFloat}
    return (b.lim - input[b.alb_idx]) * b.t_scale_factor[]::FT
end
...
for the NeuralSnow models. This should not be used for arbitrary
...
It can only be called prior to the model being made static, 
in the case of the NeuralDepthModel, or even afterwards for the NeuralAlbedoModel.
"""
function _set_timescale!(m::ConstrainedNeuralModel, Δt::Real)
    ...
    elseif typeof(m.constraints.upper_bound) <: Snow_Albedo_Bound
        m.constraints.upper_bound.t_scale_factor[] = FT(1 / Δt)
        m.constraints.lower_bound.t_scale_factor[] = FT(1 / Δt)
    else
...
snow_depth_predmodel = Chain(
    Dense(n_features, n_features*k, relu),
...
of arrays.
"""
#what happens when α param is set differently for this than the DepthModel?
struct NeuralAlbedoModel{FT, AM <: ConstrainedNeuralModel} <:
    ...
    function NeuralAlbedoModel(alb_model::AM, α::FT) where {FT, AM}
        ...
        be additionally imported into this module:
        =#
        static_alb_model = Adapt.adapt(SArray, alb_model)
...
"""
    get_albnetwork(FT)
Return the snow-albedo neural network developed by Charbonneau et al (),
with the network's scaling such that snowpack albedo remains between 0.3 and 1.
"""
function get_albnetwork(FT)
    #TODO: FIX THIS METHOD ONCE THE RIGHT ARTIFACTS ARE IN
end
