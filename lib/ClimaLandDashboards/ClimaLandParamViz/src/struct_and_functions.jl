...
function mat(
    ...
    x = collect(
        range(
            inputs.drivers.ranges[1][1],
            length = steps,
            stop = inputs.drivers.ranges[1][2],
        ),
    )
    y = collect(
        range(
            inputs.drivers.ranges[2][1],
            length = steps,
            stop = inputs.drivers.ranges[2][2],
        ),
    )
    ...
    FMatrix =
        Matrix(sparse(X, Y, parameterisation.(drivers, parameters, constants)))
    return x, y, FMatrix
end
...
function d1_vec(
    ...
    x = collect(
        range(inputs.drivers.ranges[1][1], inputs.drivers.ranges[1][2], steps),
...
function d2_vec(
    ...
    y = collect(
        range(inputs.drivers.ranges[2][1], inputs.drivers.ranges[2][2], steps),
