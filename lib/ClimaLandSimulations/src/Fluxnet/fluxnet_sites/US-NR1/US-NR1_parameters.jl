export niwotridge_default_params,
    hetero_resp_niwotridge,
    auto_resp_niwotridge,
    soil_niwotridge,
    radiative_transfer_niwotridge,
    canopy_energy_balance_niwotridge,
    conductance_niwotridge,
    photosynthesis_niwotridge,
    plant_hydraulics_niwotridge

function niwotridge_default_params(;
    hetero_resp = hetero_resp_niwotridge(),
    auto_resp = auto_resp_niwotridge(),
    soil = soil_niwotridge(),
    radiative_transfer = radiative_transfer_niwotridge(),
    canopy_energy_balance = canopy_energy_balance_niwotridge(),
    conductance = conductance_niwotridge(),
    photosynthesis = photosynthesis_niwotridge(),
    plant_hydraulics = plant_hydraulics_niwotridge(),
)
    return SiteParams(
        hetero_resp,
        auto_resp,
...
function canopy_energy_balance_niwotridge(; ac_canopy = FT(3e3))
    return BigLeafEnergyParameters(ac_canopy)
end

function conductance_niwotridge(;
...
function photosynthesis_niwotridge(;
    is_c3 = FT(1),
    oi = FT(0.209),
    ϕ = FT(0.6),
    θj = FT(0.9),
...
function plant_hydraulics_niwotridge(;
    ...
    rooting_depth = FT(1.0), # from Natan
)
    conductivity_model =
