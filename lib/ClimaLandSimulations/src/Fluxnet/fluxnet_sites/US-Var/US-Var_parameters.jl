export vairaranch_default_params,
    hetero_resp_vairaranch,
    auto_resp_vairaranch,
    soil_vairaranch,
    radiative_transfer_vairaranch,
    canopy_energy_balance_vairaranch,
    conductance_vairaranch,
    photosynthesis_vairaranch,
    plant_hydraulics_vairaranch

function vairaranch_default_params(;
    hetero_resp = hetero_resp_vairaranch(),
    auto_resp = auto_resp_vairaranch(),
    soil = soil_vairaranch(),
    radiative_transfer = radiative_transfer_vairaranch(),
    canopy_energy_balance = canopy_energy_balance_vairaranch(),
    conductance = conductance_vairaranch(),
    photosynthesis = photosynthesis_vairaranch(),
...
function soil_vairaranch(;
    vg_n = FT(2.00), # unitless
    vg_α = FT(2.00), # inverse meters
    ν = FT(0.45), # m3/m3
    ν_ss_quartz = FT(0.1),
    ν_ss_om = FT(0.1),
...
function canopy_energy_balance_vairaranch(; ac_canopy = FT(745))
    return BigLeafEnergyParameters(ac_canopy)
end

function conductance_vairaranch(;
...
function photosynthesis_vairaranch(;
    is_c3 = FT(1),
    oi = FT(0.209),
    ϕ = FT(0.6),
    θj = FT(0.9),
...
function plant_hydraulics_vairaranch(;
    SAI = FT(0.0), # m2/m2 or: estimated from Wang et al, FT(0.00242) ?
    ...
    rooting_depth = FT(2.6), # from Natan
)
    conductivity_model =
