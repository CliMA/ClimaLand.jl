...
\end{align}
```
where $h = \psi+z$ is the head (in meters), and $K$ is the conductivity (units of m/s). We approximate the flux between two depths, indexed with 2 at
...
```math
\begin{equation}
    \frac{d V}{dt} = q_{roots}\sigma_{roots} - \tau \sigma_{leaf},
\end{equation}
```
...
```math
\begin{equation}
    \frac{d v}{dt} = q_{roots}RAI - \tau LAI,
\end{equation}
```
