...
# In the [Canopy, soil, and snow]() tutorial, we ran the integrated `LandModel`
# at a Fluxnet site using all of the default parameterizations and parameters.

# In two other tutorials ([Changing soil parameterizations](@ref standalone/Soil/changing_soil_parameterizations.jl)
# and [Changing canopy parameterizations](@ref standalone/Canopy/changing_canopy_parameterizations.jl))
...
# This time, we'll use non-default parameterizations for two canopy components:
...
# - snow albedo: from the default `ConstantAlbedo` to `ZenithAngleAlbedoModel`
...
# with custom parameters. We already did this in the [Changing canopy parameterizations](@ref standalone/Canopy/changing_canopy_parameterizations.jl)
