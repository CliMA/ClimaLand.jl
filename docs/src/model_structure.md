...
In ClimaLand's framework, the land system is divided into individual components: soil, vegetation (or canopy), snow, and surface water (or pond). Each of these components can be run individually (_standalone_) or combined together (_integrated_).
...
Prognostic variables are stored in the model cache, which is represented as the variable `p`.
