...
# - `input` will be an `AbstractArray{<:AbstractFloat}` type (usually a `Matrix`), of size ``N\timesK``, where ``N`` is the number of input features and ``K`` is the number of samples provided
# - `pred` will also be an `AbstractArray{<:AbstractFloat}` type of size 1×`K`, where `K` is the number of predictions made from the provided inputs/samples
# - the boundary functions must return a row-vector of size ``1\timesK``, equal to the values of the boundary values for each input sample, or something that is readily broadcasted to this size (like a single scalar)
...
# The above example shows how to build a more general setup depending on your features, data, and model
...
# be built correctly with the correct arguments to `make_model_paper()` to 
