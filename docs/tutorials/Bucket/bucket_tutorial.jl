...
# option is to specify a `BulkAlbedoStatic` or `BulkAlbedoTemporal`, whic
# use a NetCDF file to read in surface albedo.
# These options only apply when coordinates are (lat,lon).
