...
for i in 1:N_iterations
    ...
    ClimaLand_ens = hcat([Ozark_LHF(params_i[:, i]) for i in 1:N_ensemble]...)
...
l2 = [
    lines!(
        ax,
        trange,
        Ozark_LHF(EKP.get_ϕ(prior, ensemble_kalman_process, 1)[:, i]),
        color = :red,
    ) for i in 1:N_ensemble
][1]
l3 = [
    lines!(ax, trange, Ozark_LHF(final_ensemble[:, i]), color = :green) for
    i in 1:N_ensemble
][1]
