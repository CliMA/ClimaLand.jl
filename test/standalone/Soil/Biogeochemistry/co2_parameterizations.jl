...
for FT in (Float32, Float64)
    @testset "Soil CO2 production and transport, FT = $FT" begin
        ...
        ρ_O2_air = o2_concentration(O2_a, T_soil, P_sfc)
        @test typeof(ρ_O2_air) == FT
        @test ρ_O2_air > FT(0)

        # Test inverse: converting back from concentration to fraction
        O2_a_reconstructed =
            o2_fraction_from_concentration(ρ_O2_air, T_soil, P_sfc)
        @test O2_a_reconstructed ≈ O2_a
        @test typeof(O2_a_reconstructed) == FT

        # Test O2 availability (dimensionless, accounts for tortuosity)
        (; D_oa) = parameters
        O2_avail = o2_availability(O2_a, θ_a, D_oa)
        @test O2_avail == D_oa * O2_a * θ_a^(FT(4 / 3))
