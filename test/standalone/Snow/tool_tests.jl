...
if !isnothing(SNOTELScraperExt)
    ...
    @testset "Testing ConstrainedNeuralModels module" begin
        FT = Float32
        in_module = @__MODULE__ #Running on GPU creates a submodule of Main that code is run within, not in Main.
        ...
        this_mod_set =
            Set([StaticArrays, Adapt, JLD2, Flux, Base, Core, ClimaLand])
