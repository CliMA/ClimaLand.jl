...
if !isnothing(SNOTELScraperExt)
    ...
    @testset "Testing NeuralSnow module" begin
        ...
        cam = NeuralSnow.ConstantInitialAlbedo(0.3, 0.1)
        @test typeof(cam) <: NeuralSnow.ConstantInitialAlbedo{Float64}
        @test cam.init_α == 0.3
        @test cam.reset_limit == 0.1
        @test cam(0, 0, 0) == 0.3 #functor method takes 3 unspecified args
        cam = NeuralSnow.ConstantInitialAlbedo(FT(0.4), FT(0.2))
        @test typeof(cam) <: NeuralSnow.ConstantInitialAlbedo{FT}
        @test cam.init_α == 0.4f0
        @test cam.reset_limit == 0.2f0
        @test cam(0, 0, 0) == 0.4f0 #functor method takes 3 unspecified args
        ...
        alb_model1 =
            NeuralSnow.NeuralAlbedoModel(FT, FT(34), FT(-123), earth_param_set) #lat, lon with FT
        ...
        @test alb_model1.new_alb isa NeuralSnow.ConstantInitialAlbedo{FT}
        ...
        @test alb_model1.za_solarnoon(0, DateTime("2010-01-01T01:23")) ≈
              0.5450495f0
        ...
        @test NeuralSnow.ema_step(0.2, 3.0, 4.0) == -0.2
        @test NeuralSnow.ema_value(0.2, 3.0, 4.0) ≈ 3.8  #0.2(3) + 0.8(4) = 0.6 + 3.2
        @test NeuralSnow.reset_condition(0.0, 0.0, 0.001, 1.0, 2.0) == 2.0
        @test NeuralSnow.reset_condition(0.0, 1.0, 0.001, 1.0, 2.0) == 1.0
        @test NeuralSnow.reset_condition(1.0, 0.0, 0.001, 1.0, 2.0) == 2.0
        @test NeuralSnow.reset_condition(1.0, 1.0, 0.001, 1.0, 2.0) == 2.0
        ...
        albmodel =
            NeuralSnow.NeuralAlbedoModel(FT, lat, lon, earth_param_set, Δt = Δt)
        ...
        @test Snow.extra_prog_names(depthmodel) ==
              (:surface, :surface, :surface, :surface, :surface, :surface)
        @test Snow.extra_prog_names(albmodel) == (:surface,)
        ...
        @test all(parent(dY.snow.Z) .≈ 6.4203f-7)
