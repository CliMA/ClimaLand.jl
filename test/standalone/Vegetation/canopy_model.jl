...
for FT in (Float32, Float64)
    ...
    @testset "Jacobian FT = $FT" begin
        ...
        Y.canopy.energy.T = FT(289.1)
        p_2 = deepcopy(p)
        set_initial_cache!(p_2, Y, t0)
        dY_2 = similar(Y)
        imp_tendency!(dY_2, Y, p_2, t0)
        ...
        @test parent(
            abs.(
                (
                    ...
                    ) ./ FT(0.1) .- p.canopy.energy.∂LW_n∂Tc
                ) ./ p.canopy.energy.∂LW_n∂Tc
            ),
        )[1] < 0.05
        @test parent(
            abs.(
                (
                    (p_2.canopy.energy.shf .- p.canopy.energy.shf) ./ FT(0.1) .-
                    p.canopy.energy.∂SHF∂Tc
                ) ./ p.canopy.energy.∂SHF∂Tc
            ),
        )[1] < 0.05
        @test parent(
            abs.(
                (
                    (p_2.canopy.energy.lhf .- p.canopy.energy.lhf) ./ FT(0.1) .-
                    p.canopy.energy.∂LHF∂qc .* p.canopy.energy.∂qc∂Tc
                ) ./ (p.canopy.energy.∂LHF∂qc .* p.canopy.energy.∂qc∂Tc)
            ),
        )[1] < 0.5 # Big error, I think from ∂r_ae/∂T? as CLM states, these can't be determined analytically
        @test (abs.(
            parent((dY_2.canopy.energy.T .- dY.canopy.energy.T) ./ FT(0.1)) -
            (jac_value .+ 1)
        ) / (jac_value .+ 1))[1] < 0.25 # Error propagates here
