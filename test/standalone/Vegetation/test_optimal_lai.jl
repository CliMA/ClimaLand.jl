...
@testset "Optimal LAI Model Tests" begin
    for FT in (Float32, Float64)
        ...
        @testset "compute_L_max function for FT = $FT" begin
            ...
            LAI_max = Canopy.compute_L_max(Ao_annual, P_annual, D_growing, k, z, ca, chi, f0)
            ...
            LAI_wet = Canopy.compute_L_max(Ao_annual, P_wet, D_growing, k, z, ca, chi, f0)
            
            # Dry conditions - should be water limited
            P_dry = FT(20000.0)  # Low precipitation
            LAI_dry = Canopy.compute_L_max(Ao_annual, P_dry, D_growing, k, z, ca, chi, f0)
        ...
        @testset "lambertw0 function for FT = $FT" begin
            ...
            @test Canopy.lambertw0(FT(1.0)) ≈ FT(0.5671432904097838) atol=FT(1e-6)
        ...
        @testset "update_LAI! function for FT = $FT" begin
            ...
            LAI_no_update = Canopy.update_LAI!(LAI_prev, L_steady, alpha, FT(0.0))
            @test LAI_no_update == LAI_prev
            
            # Test that LAI is non-negative
            LAI_negative_test = Canopy.update_LAI!(-FT(1.0), FT(0.0), alpha, FT(1.0))
        ...
        @testset "get_local_noon_mask function for FT = $FT" begin
            ...
            # Test within window
            mask_before = Canopy.get_local_noon_mask(43200.0 - dt/4, dt, local_noon)
            @test mask_before == FT(1.0)
            
            mask_after = Canopy.get_local_noon_mask(43200.0 + dt/4, dt, local_noon)
            @test mask_after == FT(1.0)
            
            # Test outside window
            mask_outside = Canopy.get_local_noon_mask(43200.0 + dt, dt, local_noon)
