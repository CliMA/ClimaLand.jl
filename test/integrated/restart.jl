...
@testset "Integrated land model restart test" begin
    ...
    # The restarted simulation keeps the epoch of the original one, as ClimaCoupler
    # does, so its time counts from `start_date` and not from the checkpoint.
    @test epoch(t_restart) == start_date

    restart_t0, restart_tf, restart_Δt = promote(
        t_restart,
        ITime(Dates.value(Second(stop_date - start_date)); epoch = start_date),
        ITime(Δt; epoch = start_date),
    )
    @test float(restart_t0) == n_steps_to_checkpoint * Δt
    ...
    @testset "Final state after restarting" begin
        compare_states(simulation._integrator.u, restart._integrator.u)
    end
