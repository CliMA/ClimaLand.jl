    ...
    for property in properties
        for var in propertynames(property)
            ...
            if length(size(field_values)) == 5 # 3d var
                @test extrema(field_values[:, 1, 1, :, binary_mask]) ==
                      (val, val)
            else
...
function check_ocean_values_Y(Y, binary_mask; val = 0.0)
    @test extrema(parent(Y.soil.ϑ_l)[:, 1, 1, 1, binary_mask]) == (val, val)
    @test extrema(parent(Y.soil.θ_i)[:, 1, 1, 1, binary_mask]) == (val, val)
    @test extrema(parent(Y.soil.ρe_int)[:, 1, 1, 1, binary_mask]) == (val, val)
    @test extrema(parent(Y.snow.U)[1, 1, 1, binary_mask]) == (val, val)
    @test extrema(parent(Y.snow.S)[1, 1, 1, binary_mask]) == (val, val)
    @test extrema(parent(Y.snow.S_l)[1, 1, 1, binary_mask]) == (val, val)
    @test extrema(parent(Y.canopy.energy.T)[1, 1, 1, binary_mask]) == (val, val)
    @test extrema(
