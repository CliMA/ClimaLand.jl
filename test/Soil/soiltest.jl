...
@testset "Richards equation with flux BCs" begin
    ...
    soil_domain = Column(; zlim = (zmin, zmax), nelements = nelems)
    top_flux_bc = FluxBC{FT}((p, t) -> eltype(t)(0.0))
    bot_flux_bc = FluxBC{FT}((p, t) -> eltype(t)(0.0))
    sources = ()
...
@testset "Richards equation with state BCs" begin
    saved_values = SavedValues(FT, ClimaCore.Fields.FieldVector)
    ν = FT(0.495)
    K_sat = FT(0.0443 / 3600 / 100) # m/s
    S_s = FT(1e-3) #inverse meters
    ...
    soil_domain = Column(; zlim = (zmin, zmax), nelements = nelems)
    top_state_bc = StateBC{FT}((p, t) -> eltype(t)(ν / 2))
    bot_state_bc = StateBC{FT}((p, t) -> eltype(t)(ν))
    sources = ()
...
@testset "Richards equation with flux and state BCs" begin
    saved_values = SavedValues(FT, ClimaCore.Fields.FieldVector)
    ν = FT(0.495)
    K_sat = FT(0.0443 / 3600 / 100) # m/s
    S_s = FT(1e-3) #inverse meters
    ...
    soil_domain = Column(; zlim = (zmin, zmax), nelements = nelems)
    top_flux_bc = FluxBC{FT}((p, t) -> eltype(t)(0))
    bot_state_bc = StateBC{FT}((p, t) -> eltype(t)(ν / 2))
    sources = ()
