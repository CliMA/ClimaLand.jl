...
intp_method = Interpolations.Constant()
# resolutions = ((101, 15), (202, 15), (303, 15), (404, 15))
resolutions = [(i, 15) for i in 51:25:451]
output_dir = "/home/kphan2/worktree/ClimaLand.jl/regridding/increasing_resolutions"

rmses = []

    intp_method_name = if intp_method == Interpolations.Linear()
        "linear"
    elseif intp_method == Interpolations.Constant()
        "constant"
    else
...
for resolution in resolutions
    ...
    interpolation_method = intp_method
    ...
    # filename = "bias_latest_climautilities_no_regridding_$(intp_method_name)_$nelements.png"
    # CairoMakie.save(joinpath(output_dir, filename), fig)

    close(T_atmos)
    push!(rmses, ClimaAnalysis.global_rmse(t2m_var_from_cc, t2m_var_from_nc))
end

horizontal_elts = [first(resolution) for resolution in resolutions]

f = CairoMakie.Figure(size = (600, 650))
ax = CairoMakie.Axis(f[1, 1], xlabel = "Horizontal elements", ylabel = "RMSE", title = "RMSE as a function of horizontal elements with\n$intp_method_name interpolation\nand no regridding from ClimaAnalysis", xlabelsize = 20, ylabelsize = 20, xticklabelsize = 20, yticklabelsize = 20, titlesize = 20)
CairoMakie.ylims!(ax, 0.0, 1.7)
CairoMakie.lines!(ax, horizontal_elts, rmses, linewidth = 6)
CairoMakie.save(joinpath(output_dir, "$(intp_method_name)_line_plot.png"), f)
