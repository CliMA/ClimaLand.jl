...
# In [Default Canopy](@ref standalone/Canopy/default_canopy.jl), we ran a simple canopy model simulation
...
# Alternatively, we could use the same constructor but provide custom parameters.
...
Ω = 1; # clumping index
radiation_parameters = (; G_Function, α_PAR_leaf, α_NIR_leaf, Ω);
radiative_transfer = Canopy.BeerLambertModel{FT}(domain; radiation_parameters);

# This option may seem a bit strange since we were required to provide the domain
# even though it was not used in the constructor.
