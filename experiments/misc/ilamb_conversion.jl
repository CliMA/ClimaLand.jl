...
find all the paths of the NetCDF files that can be converted to ILAMB.

This function assumes that NetCDF files that have monthly averages end with
"_1M_average.nc".
"""
...
Add the nontemporal dimensions to `ilamb_ds` using `clima_ds`.
...
Add the temporal dimensions to `ilamb_ds` using `clima_ds`.
...
function define_var(ilamb_ds, clima_ds, clima_short_name)
    ...
    (; short_name, long_name, conversion_factor, units, clima_units) =
        ilamb_mapping
    var_attribs = Dict(
...
Get the data from `clima_ds` with the name `clima_short_name`.
...
function get_data(
    ...
    clima_ds[clima_short_name].attrib["units"] != clima_units && error(
        "Units are not the same for $clima_short_name. Update the conversion factor and units in the mapping and run this script again",
    )
