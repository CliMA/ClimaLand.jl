...
function ClimaCalibrate.observation_map(iteration)
    ...
    # TODO: Replace with ClimaCalibrate.ObservationRecipe.short_names
...
function process_member_data(
    ...
    for short_name in short_names
        short_name in keys(era5_obs_vars) || error(
            "Do not know how to process member data for a variable with the short name $short_name",
        )
    end
    ...
    vars = map(short_names) do short_name
        ...
        # TODO: Replace this with mask aware flatten once it is merged in
        # ClimaAnalysis
        nanmean_land_val = 500.0
