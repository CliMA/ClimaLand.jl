# Calibration config: optimal-LAI parameters against MODIS LAI, TIGHTER NOISE.
#
# Experiment C. Identical to lai.jl except NOISE_SCALARS["lai"] is 0.25 instead
# of 0.5 - the change lai.jl's own docstring prescribes for this symptom:
# "tighten it (-> 0.25) to pull harder on peak-LAI magnitude".
#
# Why this and not more data: experiment A quadrupled the target to four annual
# cycles and reproduced the single-year parameter trajectory to three significant
# figures, finishing at masked RMSE 0.895 against the single-year run's 0.885.
# More data does not help, so the z-sigma ridge is structural.
#
# What the diagnostics point at instead. Every calibration so far drives the
# masked bias negative (+0.081 default -> -0.098 single-year -> -0.146
# four-year), `z` rises 15 -> 20-27 in every run, and larger z means LOWER peak
# LAI. Masked RMSE also peaks in NH summer (1.02-1.06) and is lowest in DJF
# (0.76-0.79), and the bias map is most negative over the Amazon and Congo. All
# three point the same way: the fit is giving up peak-season, high-LAI magnitude
# to reduce error elsewhere. Halving the assumed observation variance makes that
# trade more expensive.
#
# Baselines to beat, masked: default 0.941 / +0.081, single-year calibrated
# 0.885 / -0.098, four-year 0.895 / -0.146.
#
...
priors.
"""
const NOISE_SCALARS = Dict("lai" => 0.25)
...
const CALIBRATE_CONFIG = CalibrateConfig(;
    ...
    rng_seed = 42,
    obs_vec_filepath = "experiments/calibration/land_observation_vector_lai_noise025.jld2",
