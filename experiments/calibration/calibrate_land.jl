...
using Dates
const variable_list = ["lhf", "shf", "swu", "lwu"] # variables you want to capture by adjusting your priors
...
const start_date = DateTime(2007, 12, 01) # this is the start of the forward model spinup
...
include(
    joinpath(@__DIR__, "make_training_locations.jl"),
)
# path_to_grid not used, I believe
# path_to_grid = joinpath(caldir, "sample", "iteration_000/member_001/global_diagnostics/output_active/")
# training_locations = make_training_locations()
using JLD2
...
# Below, config for the workers - to run ClimaLand (forward model) in parallel for different members
# Note: the forward model needs to be adjusted to read priors!
@everywhere begin
    ...
    start_date = DateTime(2007,12,01)
...
caldir = "calibration_output_utki" # Where ClimaCalibrate stores outputs
