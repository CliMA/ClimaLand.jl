        ...
        NCDataset(filepath, "r") do ds
            for period in period_preference
                ...
                if haskey(ds.group, group_name)
                    ...
                    if haskey(group, "latitude") && haskey(group, "longitude")
                        lats = Array{Float64}(group["latitude"][:])
                        lons = Array{Float64}(group["longitude"][:])
                        
                        # Load soil moisture
                        if haskey(group, "soil_moisture")
                            sm = Array{Float64}(group["soil_moisture"][:])
...
function load_smap_grid_timeseries(
    ...
    # Pre-allocate 3D array [lon, lat, time]
    nx, ny = size(sm_first)
    nt = length(files)
    sm_timeseries = Array{Float64,3}(undef, nx, ny, nt)
    ...
    for (i, file) in enumerate(files[2:end])
        ...
        try
            ...
            if size(sm) != (nx, ny)
                @warn "Grid size mismatch in file: $(basename(file)), expected ($nx, $ny), got $(size(sm))"
                sm_timeseries[:, :, idx] .= NaN
            else
    ...
    @info "Loaded SMAP timeseries" n_files=nt grid_size=(nx, ny) date_range=(minimum(dates), maximum(dates))
...
Calculate temporal mean soil moisture from multiple SMAP files.
Returns the time-averaged soil moisture grid.
...
# Returns
- `Tuple{Array{Float64,2}, Array{Float64,2}, Array{Float64,2}}`: (lons, lats, sm_mean)
  - `sm_mean`: Temporal mean, with NaN where data was missing for all times
"""
function calculate_smap_temporal_mean(
    files::Vector{String}; 
    quality_flag_threshold::Int = 0
)
    lons, lats, sm_timeseries, dates = load_smap_grid_timeseries(
        files; 
        quality_flag_threshold
    )
    
    # Calculate mean along time dimension, ignoring NaN
    sm_mean = dropdims(
        mapslices(x -> isempty(filter(!isnan, x)) ? NaN : mean(filter(!isnan, x)), sm_timeseries, dims=3),
        dims=3
    )
    
    n_valid = dropdims(sum(.!isnan.(sm_timeseries), dims=3), dims=3)
    
    @info "Calculated SMAP temporal mean" total_timesteps=length(dates) mean_valid_obs=mean(n_valid) grid_coverage=count(n_valid .> 0) / length(n_valid)
    
    return (lons, lats, sm_mean)
end
