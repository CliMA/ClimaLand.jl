...
include(joinpath(pkgdir(ClimaLand), "experiments/calibration/config.jl"))

include(
    joinpath(pkgdir(ClimaLand), "experiments/calibration/observation_map.jl"),
)

@everywhere begin
    import ClimaLand
    experiment_dir = joinpath(pkgdir(ClimaLand), "experiments")
    include(joinpath(experiment_dir, "calibration", "model_interface.jl"))
    CALIBRATE_CONFIG
end
