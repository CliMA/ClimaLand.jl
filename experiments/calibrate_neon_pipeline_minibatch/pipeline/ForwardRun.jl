...
if !isdefined(@__MODULE__, :_get_neon_site_metadata)
    include(joinpath(pkgdir(ClimaLand), "experiments/calibrate_neon_pipeline_minibatch/src/site_metadata.jl"))
end
...
""")
    ...
    # ── Override Rosetta retention params with NEON per-site profiles ──────────
    ...
    # MINIBATCH: observations.jld2 no longer stores a single flat `obs_dates`; the
    # soil-CO₂ days that survived the spring-peak mask are stored PER WINDOW, PER
    # DEPTH in `obs_dates_per_window` (Vector of Dict(code => dates)). The forward
    # RMSE / correlation / obs-CO₂ means must be computed on the SAME set the
    # calibration saw, so we take the UNION of those kept dates across all windows
    # and all depths — the full set of days any minibatch window used. `obs_daily_cal`
    # is obs_daily restricted to that set; the full `obs_daily` is kept for the
    # time-series figures. `capped_co2_dates` are the in-span obs-days that were
    # dropped, used to shade the figures.
    obs_jld = joinpath(Config.base_dir_for(run), "observations.jld2")
    if isfile(obs_jld)
        _odpw = JLD2.load(obs_jld, "obs_dates_per_window")
        kept_dates = Set{Date}()
        for wdict in _odpw            # one Dict(code => dates) per kept window
            for (_code, dts) in wdict
                union!(kept_dates, Date.(dts))
            end
        end
