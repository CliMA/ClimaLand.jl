...
function seed_from_restart(run, csv_path, output_root)
    ...
    return Config.RunConfig(
        run.site, run.start_date, run.stop_date, run.windows, run.spinup_days,
        run.n_iterations, run.minibatch_size, run.max_n_samples, run.rng_seed,
        run.cal_depth, run.cal_depth_codes, run.settingsdesc,
...
function run_one(run, cfg::Config.PipelineConfig; skip_restart = false)
    ...
    println("\n" * "#"^78)
    println("# RUN  site=$(run.site)  span=$(run.start_date)..$(run.stop_date)  ",
            "windows=$(length(run.windows))  minibatch=$(run.minibatch_size)  ",
            "id=$(run.run_identifier)")
