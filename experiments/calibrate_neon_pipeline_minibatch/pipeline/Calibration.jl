...
# Path to the model-side scripts we reuse. In the AllDepth pipeline these live in
# a self-contained `src/` folder (copied from the original experiments/calibrate_neon),
# so multi-depth edits never touch the live pipeline.
const SRC_EXPERIMENT_DIR =
    joinpath(pkgdir(ClimaLand), "experiments", "calibrate_neon_pipeline_minibatch", "src")
...
function run_calibration(run; obs_filepath, output_dir)
    ...
    # ── Load observation SERIES (minibatch) ──────────────────────────────────
    # MINIBATCH: the EKP is built from an EKP.ObservationSeries (one Observation
    # per window) + a minibatcher, NOT from a single (y_obs, noise_cov) pair. Each
    # EKP iteration then draws `minibatch_size` windows; forward_model /
    # observation_map map iteration → window indices via
    # EKP.get_minibatch(series, iteration) (see model_interface_wLabile.jl).
    obs_data = JLD2.load(obs_filepath)
    obs_series = obs_data["observation_series"]
    n_windows = length(obs_data["windows"])
    println("Loaded observation series: $n_windows windows, ",
        "minibatch_size=$(obs_data["minibatch_size"]) for site $(run.site)")

    # ── Create EKP ───────────────────────────────────────────────────────────
    # Estimator kept identical to the AllDepth pipeline: Unscented(α_reg = 1.0).
    # Minibatching is a property of the observation object, not the estimator, so
    # only the first argument changes (obs_series instead of y_obs, noise_cov).
    ...
    # MINIBATCH: NO fixed NEON_START_DATE/NEON_STOP_DATE ENV anymore — the window
    # date range is derived per-iteration inside forward_model from the
    # observation series' minibatch (via OBS_FILEPATH), not from a run-level ENV.
    globals_expr = quote
    ...
    # ── Run calibration ──────────────────────────────────────────────────────
    println("\n=== Starting ClimaCalibrate calibration (MINIBATCH) ===")
    println("  Site: $(run.site)  |  Span: $(run.start_date)..$(run.stop_date)")
    println("  Iterations: $(run.n_iterations)  |  Ensemble: $N_ens  |  ",
        "Windows: $n_windows  |  minibatch_size: $(obs_data["minibatch_size"])")
    ...
    open(FINAL_PARAMS_FILE, "w") do io
        ...
        println(io, "Span: $(run.start_date) .. $(run.stop_date)  ",
            "($n_windows windows, minibatch_size=$(obs_data["minibatch_size"]))")
