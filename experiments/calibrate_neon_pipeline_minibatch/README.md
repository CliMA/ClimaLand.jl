...
## Multi-depth (AllDepth) variant

This is the **all-depth** copy of the pipeline (`calibrate_neon_pipeline_AllDepth`):
a self-contained clone of `calibrate_neon_pipeline` being adapted to calibrate on
all three NEON soil-CO₂ depths (codes 501/502/503) jointly, instead of one depth.
Model-side scripts live in `src/` here (not the shared `experiments/calibrate_neon`),
so edits stay isolated from the live single-depth pipeline.

**Per-site measurement depths → model layers** come from a lookup CSV,
`/home/evametz/Scripts/CliMA/neon_depth_layer_mapping.csv`, which is generated by
the notebook:

- [neon_depth_layer_mapping.ipynb](/home/evametz/Scripts/CliMA/neon_depth_layer_mapping.ipynb)

The notebook reads each site's `sensor_positions` file, cleans the depths
(-abs(zOffset), dedup duplicate rows, median across the 5 plots), and matches each
depth to the nearest ClimaLand layer center. `src/neon_depth_lookup.jl` reads that
CSV at runtime. **Rerun the notebook to regenerate the CSV** if the depth source or
the model grid changes.
