"""
Observation_flag_median.jl — MEDIAN-across-sensors variant of Observation_flag.jl.

Identical to Observation_flag.jl in every respect EXCEPT the per-half-hour
reduction ACROSS the (up to 5) NEON sensor plots: this file takes the spatial
MEDIAN across the sensors instead of the mean. Chosen to be robust to a single
outlier / drifting sensor. Everything downstream is unchanged:
  * the over-the-day reduction is still a MEAN (requiring ≥24 valid half-hours);
  * the inter-sensor VARIANCE (→ noise covariance) is unchanged (still a spread);
  * the cap mask reuses the same across-sensor reduction, so the masking decision
    and the calibration target are computed on the SAME (median) series;
  * observation vector shape, stacking order, JLD2 layout: all identical.

Only ONE of Observation_flag.jl / Observation_flag_median.jl may be `include`d
into Main at a time (they define the same names) — select in Pipeline.jl.

Original (mean) header, for reference:

  `generate_observations(run; base_dir)` builds the daily soil-CO₂ observation
  vector + noise covariance and saves observations.jld2, WITH a
  late-winter→spring "cap" mask applied to the soil-CO₂ observations.

  Mask definition (matching the Python prototype
  plot_neon_springpeak_allyears_uniondepth_batch.py):
    * Operate on DAILY data (plot-reduction per half-hour → daily mean requiring
      ≥24 valid half-hours), exactly the series that becomes `y_obs`.
    * A day is *capped* when soil CO₂ is ELEVATED (> CO2_FACTOR × baseline) AND
      the soil is COLD (daily soil-T < COLD_SOIL_K; where soil-T is missing, fall
      back to daily air-T < AIRT_FREEZE_K).
    * BASELINE is temperature-conditioned and computed ONCE over ALL YEARS: the
      25% quantile of daily soil CO₂ over all cold-soil days.
...
dates. Builds the SAME plot-reduction → daily(≥24) series as the observation
vector (MEDIAN across sensors), then flags elevated-and-cold days. Temperatures
converted °C → K. Returns an empty set if the depth's CO₂ columns are absent or
there are too few cold-soil days to define a baseline.
"""
function capped_dates(obs_df, datetimes; depth::AbstractString)
    ...
    # MEDIAN across the (up to 5) sensor plots at one half-hour (skip missing/NaN).
    function rowmedian_skipinvalid(row, cols)
        vals = Float64[]
        for c in cols
            v = row[c]
    ...
    df = DataFrame(
        date = Date.(datetimes),
        sco2 = [rowmedian_skipinvalid(row, co2_cols) for row in eachrow(obs_df)],
        soilt = isempty(soilt_cols) ? fill(NaN, nrow(obs_df)) :
                [rowmedian_skipinvalid(row, soilt_cols) for row in eachrow(obs_df)],
...
# Row-wise MEDIAN / variance across plot columns (skip missing/NaN). Module-level
# so both the per-depth daily builder and the cap mask can reuse them.
#
# The per-half-hour reduction across the (up to 5) sensor plots is the spatial
# MEDIAN (robust to a single outlier sensor); this is the ONLY difference from
# Observation_flag.jl. The over-the-day reduction (in daily_sco2_for_depth) stays
# a mean. The name is kept as `_rowmean_skipinvalid` so the call site is identical.
...
Plot-MEDIAN → daily(≥24 valid half-hours) soil-CO₂ mean + inter-sensor variance
for one `depth` code, trimmed to (spinup, stop], NaN-day-free, and with the shared
...
soil-CO₂ spring-peak cap mask applied. Sensors are reduced ACROSS plots by the
MEDIAN (see file header); everything else matches Observation_flag.jl.
...
function generate_observations(run; base_dir)
    ...
    println("Sensor reduction ACROSS plots: MEDIAN (Observation_flag_median.jl)")
