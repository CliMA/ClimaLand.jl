"""
ForwardRun_median.jl — MEDIAN-across-sensors variant of ForwardRun.jl.

Identical to ForwardRun.jl in every way EXCEPT the spatial reduction across the
(up to 5) NEON sensor plots for the soil-CO₂ OBS target: this file takes the
across-sensor MEDIAN instead of the mean (`rowmedian_skipinvalid`), so the
forward soil-CO₂ obs series (obs_daily / obs_daily_cal → forward_rmse_sco2,
forward_corr_*, forward_obs_sco2_*) is computed on the SAME reduction as the
calibration target when Observation_flag_median.jl is the active observation
operator. SWC and soil-temperature obs are left as the mean (unchanged), and the
5 individual sensor lines in the optimized_vs_sensors figure/CSV are untouched.

Only ONE of ForwardRun.jl / ForwardRun_median.jl may be `include`d — switch the
`include(".../ForwardRun.jl")` line in Pipeline.jl to pick the sensor reduction.
Both define `forward_run` with an identical signature, so the call site in
Pipeline.jl is unchanged.

Original (mean) header, for reference:

  ForwardRun.jl — `forward_run(run; output_dir, params)` runs the land model once
  at a GIVEN parameter set and writes the diagnostic figures. `params` is the
  parameter values to run at (typically the calibrated posterior means);
  `labile_depth_scale` defaults to 0.0 when absent.

Returns `(; figures_dir, scatter_stats)`. This file is `include`d into Main by
the driver.
"""
...
function forward_run(run; output_dir, params)
    ...
    println("forward_run (soil-CO₂ obs sensor reduction: MEDIAN)")
...
""")
    ...
    # Used ONLY for the soil-CO₂ obs target below, to match the calibration when
    # Observation_flag_median.jl is active. SWC/Tsoil keep rowmean_skipinvalid.
    ...
    for code in depth_codes
        ...
        # MEDIAN across sensors (only difference from ForwardRun.jl) → matches the
        # calibration target from Observation_flag_median.jl. The per-day reduction
        # below (mean of the ≥24 valid half-hours) is the SAME temporal aggregation
        # as the mean variant — only the across-sensor reduction changed.
        obs_df[!, :sco2_mean] = [rowmedian_skipinvalid(row, co2_cols) for row in eachrow(obs_df)]
