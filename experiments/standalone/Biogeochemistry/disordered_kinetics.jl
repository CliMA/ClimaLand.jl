...
sources = (AbstractSoilCarbonSource{FT},);
...
function init_model!(Y, p, model)
    function calc_k(mu::FT, sigma::FT) where {FT}
        C = FT(0.0)
        return FT(C)
    end
    p.soilC.ks .= collect(exp.(range(model.parameters.mu-8*model.parameters.sigma,model.parameters.mu+8*model.parameters.sigma,length=model.npools)));
end
