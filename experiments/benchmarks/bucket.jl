...
function setup_prob(t0, tf, Δt; nelements = (200, 7))
    ...
    driver_cb = ClimaLand.DriverUpdateCallback(updatefunc, 3Δt, t0)
    cb = CTS.CallbackSet(driver_cb)
...
if profiler == "flamegraph"
    CTS.solve(prob, ode_algo; dt = Δt, callback = cb)
    ...
    if ClimaComms.device() isa ClimaComms.CUDADevice
        ...
        prob, ode_algo, Δt, cb = setup_simulation()
        Profile.Allocs.@profile sample_rate = 0.1 CTS.solve(
            prob,
            ode_algo;
            dt = Δt,
            callback = cb,
