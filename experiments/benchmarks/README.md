...
This folder contains scripts that set up and benchmark/profile certain ClimaLand models.
...
If run on buildkite, the mean simulation
time is compared to the previous best time for that device. If it is significantly slower,
the test fails.

### Usage with an external profiler

If an external CUDA profiler is detected, the simulation is profiled for three steps,
