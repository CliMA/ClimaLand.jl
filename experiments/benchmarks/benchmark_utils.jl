...
function profile_and_benchmark(
    ...
    @info "$model_name set up took $setup_time (s)"
    ...
    use_external_profiler =
        device isa ClimaComms.CUDADevice && CUDA.Profile.detect_cupti()
        ...
        if buildkite_slug == "climaland-benchmark" ||
           buildkite_slug == "climaland-dot-jl-short-benchmarks"
            if average_timing_s > reference_time + std_timing_s
                ...
                extra_note_string *= "Please use nvtop and squeue to check. If an unexpected "
                extra_note_string *= "regression is caused by your changes, please consult with"
                extra_note_string *= "a software team member about how to proceed"
...
is true, the output should be handled by the external profiler, Otherwise, the following are
saved in `outdir`:
