...
function profile_and_benchmark(
    ...
    # setup and run once just to ensure everything is compiled
    ...
    if !use_external_profiler
        ...
        if get(ENV, "BUILDKITE_PIPELINE_SLUG", nothing) == "climaland-benchmark"
            ...
            @testset "Performance" begin
                @test reference_time - 2std_timing_s <=
                      average_timing_s <=
                      reference_time + 2std_timing_s
            end
        end
    else
        run_profiler(setup_simulation, device, outdir)
...
Use the Profile package to profile solving the simulation returned by `setup_simulation`.
...
function run_timing_benchmarks(
    ...
    while (time() - time_now) < MAX_PROFILING_TIME_SECONDS &&
        ...
        if device isa ClimaComms.CUDADevice
            ...
            push!(
                timings_s,
                ClimaComms.@elapsed device step_N_times!(simulation, 10)
            )
        end
    end
