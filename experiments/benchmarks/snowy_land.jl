...
function setup_prob(
    ...
    prob = CTS.ODEProblem(
        CTS.ClimaODEFunction(
            T_exp! = exp_tendency!,
            T_imp! = CTS.ODEFunction(imp_tendency!; jac_kwargs...),
            dss! = ClimaLand.dss!,
        ),
        Y,
        (t0, tf),
    ...
    updateat = 3600.0 * 3
    cb = ClimaLand.DriverUpdateCallback(updatefunc, updateat, t0)
    return prob, cb
end
...
if profiler == "flamegraph"
    ...
    CTS.solve(prob, ode_algo; dt = Δt, callback = cb, adaptive = false)
    ...
    while (time() - time_now) < MAX_PROFILING_TIME_SECONDS &&
        ...
        local lprob, lode_algo, lΔt, lcb = setup_simulation()
        push!(
            timings_s,
            ClimaComms.@elapsed device CTS.solve(
                lprob,
                lode_algo;
                dt = lΔt,
                callback = lcb,
    ...
    if ClimaComms.device() isa ClimaComms.CUDADevice
        ...
        p = CUDA.@profile CTS.solve(lprob, lode_algo; dt = lΔt, callback = lcb)
        ...
        Profile.@profile CTS.solve(prob, ode_algo; dt = Δt, callback = cb)
        ...
        prob, ode_algo, Δt, cb = setup_simulation()
        Profile.Allocs.@profile sample_rate = 0.0025 CTS.solve(
            prob,
            ode_algo;
            dt = Δt,
            callback = cb,
...
elseif profiler == "nsight"
    prob, ode_algo, Δt, cb = setup_simulation(; greet = true)
    integrator = CTS.init(prob, ode_algo; dt = Δt, callback = cb)
    CTS.step!(integrator)
    CTS.step!(integrator)
    CTS.step!(integrator)
    CTS.step!(integrator)
