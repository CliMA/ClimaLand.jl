...
simulation =
    LandSimulation(start_date, stop_date, Δt, land; outdir, diagnostics);
solve!(simulation)
...
water_volume_per_area_change =
    diagnostics[1].output_writer.dict["wvpac_10d_inst"]
...
for i in 1:2
    ...
    CairoMakie.save(joinpath(outdir, "$(quantity_names[i]).png"), fig_cycle)
end
