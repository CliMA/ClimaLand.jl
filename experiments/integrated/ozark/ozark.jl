...
for (float_type, tf) in ((Float32, 3 * dt), (Float64, tf))
    ...
    # Post-process Float64 simulation only
    if FT == Float64
        ...
        data_daily_indices =
            range(0, step = DATA_DT / 3600, length = data_daily_points)
        model_daily_indices =
        ...
        function diurnal_avg(series)
            ...
            daily_avgs = [
                mean([daily_data[i][j] for i in 1:num_days]) for
                j in 1:daily_points
            ]
            return daily_avgs
        end
        ...
        # SW_OUT
        SW_out_model =
            [parent(sv.saveval[k].SW_out)[1] for k in 1:length(sv.saveval)]
        SW_out_model_avg = diurnal_avg(SW_out_model)
        SW_out_data_avg = diurnal_avg(
            FT.(SW_OUT)[Int64(t_spinup ÷ DATA_DT):Int64(tf ÷ DATA_DT)],
        )

        RMSD = StatsBase.rmsd(
            SW_out_model_avg,
            SW_out_data_avg[1:data_per_model:end],
        )
        R² =
            Statistics.cor(
                SW_out_model_avg,
        ...
        # LW_OUT
        LW_out_model =
            [parent(sv.saveval[k].LW_out)[1] for k in 1:length(sv.saveval)]
        LW_out_model_avg = diurnal_avg(LW_out_model)
        LW_out_data_avg = diurnal_avg(
            FT.(LW_OUT)[Int64(t_spinup ÷ DATA_DT):Int64(tf ÷ DATA_DT)],
        )


        RMSD = StatsBase.rmsd(
            LW_out_model_avg,
            LW_out_data_avg[1:data_per_model:end],
        )
        R² =
            Statistics.cor(
                LW_out_model_avg,
        ...
        measured_T =
            LE ./ (LSMP.LH_v0(earth_param_set) * 1000) .* (1e3 * 24 * 3600)

        ET_avg_model = diurnal_avg(T .+ E)
        ET_avg_data = diurnal_avg(
            measured_T[Int64(t_spinup ÷ DATA_DT):Int64(tf ÷ DATA_DT)],
        )
        ...
        # Water stress factor
        β = [
            parent(sv.saveval[k].canopy.hydraulics.β)[1] for
            k in 1:length(sol.t)
        ]
        plt1 = Plots.plot(
            size = (1500, 400),
        ...
        lwp = [
            parent(sv.saveval[k].canopy.hydraulics.ψ)[2] * 9800 for
            k in 1:length(sol.t)
        ]
        swp = [
            parent(sv.saveval[k].canopy.hydraulics.ψ)[1] * 9800 for
            k in 1:length(sol.t)
        ]
        ψ_soil = [
            sum(
        ...
        soil_T_5 =
            [parent(sv.saveval[k].soil.T)[end - 1] for k in 1:length(sol.t)]
        soil_T_5_avg = diurnal_avg(soil_T_5)
        soil_T_sfc =
            [parent(sv.saveval[k].soil.T)[end] for k in 1:length(sol.t)]
        ...
        TA_avg =
            diurnal_avg(FT.(TA)[Int64(t_spinup ÷ DATA_DT):Int64(tf ÷ DATA_DT)])
        TS_avg =
            diurnal_avg(FT.(TS)[Int64(t_spinup ÷ DATA_DT):Int64(tf ÷ DATA_DT)])
        ...
        G_model = [
            (
                ...
                parent(sv.saveval[k].soil_lhf)[1] -
        ...
        G_data_avg =
            diurnal_avg(FT.(G)[Int64(t_spinup ÷ DATA_DT):Int64(tf ÷ DATA_DT)])
        Rn = (SW_IN .- SW_OUT) .+ (LW_IN .- LW_OUT)
        G_alternate_data_avg = diurnal_avg(
            FT.(H .+ LE .- Rn)[Int64(t_spinup ÷ DATA_DT):Int64(tf ÷ DATA_DT)],
        )
        HplusL_avg = diurnal_avg(
            FT.(H .+ LE)[Int64(t_spinup ÷ DATA_DT):Int64(tf ÷ DATA_DT)],
        )
        RminusG_avg = diurnal_avg(
            FT.(Rn .- G)[Int64(t_spinup ÷ DATA_DT):Int64(tf ÷ DATA_DT)],
        )

        plt1 =
            Plots.plot(data_daily_indices, -1 .* G_data_avg, label = "Data: -G")
        Plots.plot!(
            plt1,
            data_daily_indices,
        ...
        r_ae = [
            parent(sv.saveval[k].canopy.energy.r_ae)[1] for k in 1:length(sol.t)
        ]
        r_ae = min.(r_ae, 1e4)
