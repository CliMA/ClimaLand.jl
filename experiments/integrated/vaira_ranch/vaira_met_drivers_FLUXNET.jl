...
dataset_path = "/Users/katherinedeck/Downloads/AMF_US-Var_FLUXNET_FULLSET_2000-2021_3-5"
data = joinpath(dataset_path, "AMF_US-Var_FLUXNET_FULLSET_HH_2000-2021_3-5.csv");
...
subset =  indices[(LOCAL_DATETIME .>= Dates.DateTime("200301010000", "yyyymmddHHMM")) .& (Dates.Year.(LOCAL_DATETIME) .<=Dates.Year(2003))]
...
# LAI data
lai_datapath = joinpath(dataset_path, "US-Var_fSun_Vcmax_DD.csv")
...
subset =  indices[(LAI_DATETIME .>= Dates.DateTime("200301010000", "yyyymmddHHMM")) .& (Dates.Year.(LAI_DATETIME) .<=Dates.Year(2003))]
...
LAI_seconds = FT.(0:LAI_DATA_DT:((length(LAI_DATETIME) - 1) * LAI_DATA_DT));
