...
savedir =
    joinpath(climaland_dir, "experiments/integrated/fluxnet/vaira_soilsnow")
mkpath(savedir)
...
ax_alb = Axis(
    ...
    ylabel = "α_soil",
    xlabel = "Days",
    title = "White-sky Soil Albedo (OfflineLinearSoilAlbedo)",
)
lines!(
    ax_alb,
    sv_times ./ 24 ./ 3600,
...
lines!(
    ...
    max.(abs.(E_measured[2:end] .- E_measured[1] .- ΔE_expected) ./ mean(E_measured), eps(Float64)),
)
...
lines!(
    ...
    max.(abs.(W_measured[2:end] .- W_measured[1] .- ΔW_expected) ./ mean(W_measured), eps(Float64)),
)
