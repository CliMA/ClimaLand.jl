...
const FT = Float64
site_ID = "NEON-cper"
savefolder = "20260303_wSOCprofil_const_2019_localdomain_noEax_newPrior_comparison"
...
# Write chosen prior values to file
priors_file = joinpath(savedir, "prior_values.txt")
open(priors_file, "w") do io
    println(io, "=== Prior Distributions ===")
    println(io, "Format: constrained_gaussian(name, mean, std, lower, upper)")
    println(io, "")
    for p in prior_configs
        println(io, "  $(p.name) : mean = $(p.mean), std = $(p.std), bounds = [$(p.lower), $(p.upper)]")
    end
    println(io, "")
    println(io, "UKI settings:")
    println(io, "  N_iterations = $N_iterations")
    println(io, "  rng_seed     = $rng_seed")
    println(io, "  spinup_days  = $spinup_days")
    println(io, "  dt           = $dt")
    println(io, "  site_ID      = $site_ID")
    println(io, "  start_date   = $start_date")
    println(io, "  stop_date    = $stop_date")
end
println("Saved prior values to: $priors_file")
