...
lat = FT(55.875) # degree
long = FT(-98.375) # degree

# Soil parameters
soil_ν = FT(0.42) # m3/m3
soil_K_sat = FT(0.3/ 3600 / 100) # m/s,X unit is cm/hr where X/3600/100 = m/s
soil_S_s = FT(1e-3) # 1/m, guess
soil_vg_n = FT(1.3) # unitless
soil_vg_α = FT(1.7) # inverse meters
θ_r = FT(0.067) # m3/m3, 
...
K_sat_plant = 7e-8 # m/s # seems much too small?
...
rooting_depth = FT(1.5) # from Bonan Table 2.3
z0_m = FT(2) * h_canopy
z0_b = FT(0.15) * z0_m
