# ── DK-Sor CalMIP Phase 1a: Default LandModel Configuration ──────────────────
#
# This script runs the DK-Sor (Denmark Sorø) CalMIP Phase 1a site using the
# high-level LandModel constructor, which auto-configures all sub-models
# (soil, canopy, snow, soilCO2) from toml/spatial defaults.
#
# Default sub-models:
#   - Photosynthesis: FarquharModel
#   - Conductance: MedlynConductanceModel
#   - Radiative transfer: TwoStreamModel
#   - Soil moisture stress: TuzetMoistureStressModel
...
outdir = joinpath(
    pkgdir(ClimaLand),
    "experiments/integrated/fluxnet/$(site_ID)/callmip_default/out",
)
mkpath(outdir)
