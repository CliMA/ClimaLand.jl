...
function run_model(α_sx_val, Ea_sx_val, kM_sx_val, kM_o2_val)
    ...
    # Aditionally comment out to fall back on global parameters
    #photosynthesis_parameters = (; is_c3 = FT(1), Vcmax25)
    photosynthesis =
        FarquharModel{FT}(canopy_domain, toml_dict)#; photosynthesis_parameters) Aditionally comment out to fall back on global parameters
