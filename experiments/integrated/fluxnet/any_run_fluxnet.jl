...
"""
    run_helper(site_ID, params)

Helper function to simulate a given FLUXNET site with optional custom parameters.
This function allows the drivers to be invoked at the latest world iteration during
simulation and visualizes the resulting outputs in the corresponding site's folder.
"""
# helper wrapper function to invoke on latest runs
function run_helper(site_ID, params)
    # update params if customized
    if params != nothing
        for (k, v) in params
            @eval $(k) = $v
        end
    ...
    # Plotting
    savedir =
        generate_output_path("experiments/integrated/fluxnet/$site_ID/out/")
...
"""
    run_single_site(site_ID, params::Union{Nothing, Dict{Symbol, Any}} = nothing, specific_flag::Bool = true)

Wrapper function to run a simulation for a given FLUXNET site with optional custom parameters 
and optional site-specific conditions. The optional parameters must be received in Dict format. 
"""
function run_single_site(  # e.g., run_single_site(
    site_ID, # "US-MOz",
