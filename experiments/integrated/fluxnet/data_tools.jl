...
function replace_poor_by_mean!(field, flag, bounds::Vector)
    ...
    """
    this function is very similar to "replace_poor_by_mean!" replaces field with flag values out of bounds range with average of field values whose flag are within the range
        # Arguments
...
"""
    replace_missing_with_mean_by_value!(field)
...
function replace_with_longterm_mean!(
    ...
    function check_and_replace(criteria, field, flag, formatted_time)
        ...
        if any(flg -> flg >= bounds[1] && flg <= bounds[2], filtered_flags)
            # Assuming replace_poor_by_mean! modifies `field` in place for matching indices
