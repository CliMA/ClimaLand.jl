...
θ1 = [parent(sol.u[k].soil.ϑ_l .+ sol.u[k].soil.θ_i)[end] for k in 1:length(sol.t)];
θ2 = [parent(sol.u[k].soil.ϑ_l .+ sol.u[k].soil.θ_i)[end-2] for k in 1:length(sol.t)];
θ3 = [parent(sol.u[k].soil.ϑ_l .+ sol.u[k].soil.θ_i)[end - 5] for k in 1:length(sol.t)];
...
data_id_post_spinup = Array(Int64(t_spinup ÷ DATA_DT + 1):1:Int64(tf ÷ DATA_DT + 1))
...
SW_n = [
    parent(
        sv.saveval[k].drivers.SW_d .- sv.saveval[k].SW_u)[1] for k in 1:length(sol.t)
];
LHF = [
    parent(
...
SW_n_model_monthly, _ = compute_monthly_avg(SW_n, model_times, ref_time)
...
SW_n_data_monthly, _ = compute_monthly_avg(SW_n_data, model_times, ref_time)
...
lines!(ax4, Array(1:1:12), SW_n_model_monthly, label = "Model", color = "blue")
lines!(ax4, Array(1:1:12), SW_n_data_monthly, label = "Data", color = "orange")
...
lines!(ax3, Array(1:1:12), SHF_model_monthly, label = "", color = "blue")
lines!(ax3, Array(1:1:12), SHF_data_monthly, label = "", color = "orange")
ax2 = Axis(
    fig[2, 1],
    xlabel = "",
    ylabel = "LHF (W/m²)",
...
lines!(ax2, Array(1:1:12), LHF_model_monthly, label = "", color = "blue")
lines!(ax2, Array(1:1:12), LHF_data_monthly, label = "", color = "orange")
...
lines!(ax1, Array(1:1:12), GPP_model_monthly, label = "", color = "blue")
lines!(ax1, Array(1:1:12), GPP_data_monthly, label = "", color = "orange")
...
text!(ax0, 340, 310,text = "2 cm")
lines!(ax0, model_times ./ 3600 ./ 24, T_soil1, label = "", color = "blue")
...
text!(ax1, 340, 310, text = "32 cm")
lines!(ax1, model_times ./ 3600 ./ 24, T_soil5, label = "", color = "blue")
...
text!(ax2, 340, 0, text = "0-2 cm")
lines!(ax2, model_times ./ 3600 ./ 24, θ1, color = "blue", label = "")
lines!(
    ax2,
    data_times ./ 3600 ./ 24,
...
text!(ax3, 340, 0, text = "20 cm")
lines!(ax3, model_times ./ 3600 ./ 24, θ3, color = "blue", label = "Model")
lines!(
    ax3,
    data_times ./ 3600 ./ 24,
