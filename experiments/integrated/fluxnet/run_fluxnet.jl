...
if (length(ARGS) > 1) #only plot if a second ARGS "save" is passed when running the code
    if ARGS[2] == "save"
        # Assuming plt1 is already created as shown in previous examples
        hourly_range = 0.5:0.5:24  # Common x-axis values
        dpi_res = 250#resolution of dpi
        ...
        # Define a structure for plot configurations
        struct PlotConfig
            title::String
            model_data::Any
            data_condition::Any
            measured_data_transform::Function
            units::String
        ...
        plot_configs = [
            ...
            PlotConfig(
                "ET",
                ET_model,
                drivers.LE.status == absent,
                (x) ->
                    x ./ (LP.LH_v0(earth_param_set) * 1000) .*
                    (1e3 * 24 * 3600),
                "mm/day",
                plot_daily_avg,
            ),
        ...
        # Function to extract measured data based on time frame
        extract_measured_data =
            (config, drivers) -> begin
                data_key = Symbol(config.title)
                driver = getfield(drivers, data_key)
                driver.values[Int64(t_spinup ÷ DATA_DT):Int64(tf ÷ DATA_DT)]
            end

        # Iterate through configurations and generate plots
        for config in plot_configs
            if config.data_condition
                config.plot_function(
                    config.title,
                    config.model_data,
                    dt * n,
                    num_days,
                ...
                measured_data = config.measured_data_transform(
                    extract_measured_data(config, drivers),
                )
                plot_avg_comp(
                    config.title,
                    config.model_data,
        ...
        plot_configs = [
            (
                ...
                filename = "moisture_stress.png",
            ),
            (
                data = g_stomata,
                title = "Stomatal conductance (mol/m^2/s)",
                filename = "stomatal_conductance.png",
            ),
        ]

        # Loop through each plot configuration and generate/save the plot
        for config in plot_configs
            plt1 = plot(
                daily,
                config.data,
            ...
            savefig(plt1, joinpath(savedir, config.filename))
        end
        ...
        data_series = [
            (TA_avg, "Tair (data)"),
            (soil_T_5_avg, "Tsoil (model; 5cm)"),
            (soil_T_10_avg, "Tsoil (model; 11cm)"),
        ]
        for (data, label) in data_series
            plot!(plt1, hourly_range, data, label = label, dpi = dpi_res)
        end
        plot!(
            plt1,
            xlabel = "Hour of day",
        ...
        savefig(plt1, joinpath(savedir, "soil_temperature.png"))
        #temperature
        plt1 = Plots.plot(size = (1500, 400))
        if drivers.TS.status != absent
            Plots.plot!(
        ...
        data_series_always = [
            (TA_avg, "Atmos-D"),
            (soil_T_sfc_avg, "Soil-M-2.5cm"),
            (canopy_T_avg, "Canopy-M"),
        ]

        # Loop through each always-included data series and plot
        for (data, label) in data_series_always
            plot!(plt1, hourly_range, data, label = label, dpi = dpi_res)
        end
        savefig(plt1, joinpath(savedir, "temperature.png"))
    end
end
