...
snow_frac = snowsplit.(TA, RH)
P[P .== maximum(P)] .= maximum(P[P .< maximum(P)]) # clip very large point
P_liq = -P[:] .* (1 .- snow_frac)
P_snow = -P[:] .* snow_frac

arm_seconds = FT.(1800:arm_DATA_DT:(length(arm_UTC_DATETIME) * arm_DATA_DT));
liquid_precip = TimeVaryingInput(arm_seconds, P_liq; context) # m/s
snow_precip = TimeVaryingInput(arm_seconds, P_snow; context) # m/s
atmos_q = TimeVaryingInput(arm_seconds, q[:]; context) # m/s
atmos_T = TimeVaryingInput(arm_seconds, TA[:]; context) # m/s
atmos_p = TimeVaryingInput(arm_seconds, PA[:]; context) # m/s
atmos_u = TimeVaryingInput(arm_seconds, WS_2m[:]; context) # m/s
atmos_h = FT(2)
...
atmos_co2 = TimeVaryingInput(seconds, CO2[:]; context)
LW_IN_tvi = TimeVaryingInput(seconds, LW_IN[:]; context)
SW_IN_tvi = TimeVaryingInput(seconds, SW_IN[:]; context)
