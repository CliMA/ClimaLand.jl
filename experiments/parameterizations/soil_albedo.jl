...
soc = SpaceVaryingInput("../ClimaArtifacts/soilgrids/soilgrids_nc/soilgrids_nc/soc_0-5cm_mean_5000.nc", "Band1", surface_space; regridder_type,     compose_function = (x) -> fillmissing.(x))
bdod = SpaceVaryingInput("../ClimaArtifacts/soilgrids/soilgrids_nc/soilgrids_nc/bdod_0-5cm_mean_5000.nc", "Band1", surface_space; regridder_type,     compose_function = (x) -> fillmissing.(x))
clay = SpaceVaryingInput("../ClimaArtifacts/soilgrids/soilgrids_nc/soilgrids_nc/clay_0-5cm_mean_5000.nc", "Band1", surface_space; regridder_type,     compose_function = (x) -> fillmissing.(x))
silt = SpaceVaryingInput("../ClimaArtifacts/soilgrids/soilgrids_nc/soilgrids_nc/silt_0-5cm_mean_5000.nc", "Band1", surface_space; regridder_type,     compose_function = (x) -> fillmissing.(x))
sand = SpaceVaryingInput("../ClimaArtifacts/soilgrids/soilgrids_nc/soilgrids_nc/sand_0-5cm_mean_5000.nc", "Band1", surface_space; regridder_type,     compose_function = (x) -> fillmissing.(x))
cfvo = SpaceVaryingInput("../ClimaArtifacts/soilgrids/soilgrids_nc/soilgrids_nc/cfvo_0-5cm_mean_5000.nc", "Band1", surface_space; regridder_type,     compose_function = (x) -> fillmissing.(x))
...
tcc_field = ClimaCore.Fields.zeros(surface_space)
...
names = ["albedo", "lai", "θ", "snow", "tcc", "mask"]
for (i,field) in enumerate([albedo_field, lai_field, θ_field, snow_field, tcc_field, mask])
    name = names[i]
    clims = extrema(field)
    fig = Figure()
    ax1 = Axis(fig[1,1])
...
for (i,t) in enumerate(θ.data_handler.available_dates)
    ...
    evaluate!(tcc_field, tcc, t)
    @. mask = ! ((snow_field ≈ 1.0) || (tcc_field ≈ 1.0) || (θ_field < eps(Float32)) || (lai_field > 0.1) || (albedo_field < 0.1))
...
feature_id = 1:14
nfeatures = size(feature_id)[1]
target_id = [nfeatures+1]
data_normed[:,feature_id] .= (data[:, feature_id] .- mean(data[:, feature_id], dims = 1)) ./ std(data[:, feature_id], dims = 1)
# train test split]
ndata = length(data[:,1])
(train, test) = splitobs(transpose(data_normed), at=0.7, shuffle= true);
input = Float32.(train[feature_id,:])
truth = Float32.(train[target_id,:])
test_truth = FT.(test[target_id,:])
test_input = FT.(test[feature_id,:])
...
losses_train = []
losses_test = []
@showprogress for epoch in 1:15
...
poor_count_train = mean(abs.(train_pred .- truth) .>0.1)
@show sqrt(mean((train_pred .- truth).^2))
