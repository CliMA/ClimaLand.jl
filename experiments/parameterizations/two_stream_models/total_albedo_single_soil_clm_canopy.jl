...
latg, long, la_grid = put_on_grid(lat, lon, leaf_albedo[:])
latg, long, lt_grid = put_on_grid(lat, lon, leaf_tau[:])
...
scatter!(ax, truth[:], pred[:], markersize = 0.8)
lines!(ax, [0.2,0.2], [0.8,0.8])
ax = CairoMakie.Axis(fig[2,1])
scatter!(ax, truth[:], pred[:] .- truth[:], markersize = 0.8)
CairoMakie.save("experiments/parameterizations/two_stream_models/single_soil/scatter.png", fig)
