...
(; is_c3, α_PAR_leaf, τ_PAR_leaf, α_NIR_leaf, τ_NIR_leaf) = clm_parameters
...
rg = InterpolationsRegridder(surface_space)
...
diffuse_fraction = TimeVaryingInput(
    ...
    start_date,
    regridder_type,
    compose_function = (ssrd, fdir) ->
        (ssrd .- fdir) ./ max.(ssrd, eps(Float32)),
    method = time_interpolation_method,
)
...
modis_albedo =  TimeVaryingInput(
    "../modis_MCD43A43-006.refl.00833deg_regrid.8d.2008.nc",
    "refl_band4",
    surface_space;
    start_date,
    compose_function = (x) -> clean_modis.(x),
    regridder_type,
    method = time_interpolation_method,
)
...
for (i, t) in enumerate(θ.data_handler.available_dates)
    evaluate!(albedo_field, modis_albedo, t)
        ...
        max.(
            cos.(
                zenith_angle.(
                    ...
                    lon,
                    start_date,
                    Ref(earth_param_set.insol_params),
                )
            ),
            0,
        )
...
net_features[:, 1:(end - 1)] .=
    (
        net_features[:, 1:(end - 1)] .-
        mean(net_features[:, 1:(end - 1)], dims = 1)
    ) ./ std(net_features[:, 1:(end - 1)], dims = 1);

open(joinpath(outpath, "net_features_modis.csv"); write = true) do f
    write(
        f,
        "θ, 1/α, n-1, log10(K_sat), ν, cfvo, clay, sand, silt, bdod, soc, maxLAI, ν_ss_om, ν_ss_gravel, ν_ss_quartz, LAI, P, T, vpd, diffuse_frac, c3,μ, albedo\n",
    )
    writedlm(f, net_features, ',')
end
open(joinpath(outpath, "nonnormed_input_modis.csv"); write = true) do f
    write(f, "θ,  LAI,diffuse_frac,μ\n")
    writedlm(f, nonnormed_input, ',')
end

open(joinpath(outpath, "clm_albedo_modis.csv"); write = true) do f
    write(
        f,
        "α_PAR_leaf, α_NIR_leaf, τ_PAR_leaf, τ_NIR_leaf,PAR_α_wet, PAR_α_dry, NIR_α_wet, NIR_α_dry \n",
    )
    writedlm(f, clm_albedos, ',')
end
open(joinpath(outpath, "metadata_modis.csv"); write = true) do f
    write(f, "lat, lon \n")
    writedlm(f, metadata, ',')
end
