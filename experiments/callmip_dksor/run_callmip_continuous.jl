...
const TASK =
    parse(Int, get(ENV, "CALLMIP_TASK", get(ENV, "SLURM_ARRAY_TASK_ID", "0")))
...
function runcont(ov)
    Yspin = run_prior_year(
        Y0;
        stop_year = Y0 + SPINUP_YEARS - 1,
        with_co2 = true,
        dt = 900.0,
        param_overrides = ov,
        return_state = true,
...
if TASK == 0
    ...
    out = joinpath(SIMDIR, "prior");
    mkpath(out)
    jldsave(
        joinpath(out, "callmip_diagnostics.jld2");
        dates = collect(dates),
        surface_data = surf,
        column_data = col,
        z_soil = z,
...
elseif TASK == 1
    ...
    cp = ps["constrained_posterior"];
    pn = string.(ps["param_names"])
    n_mcmc = size(cp, 2)
    ...
    out = joinpath(SIMDIR, "members");
    mkpath(out)
    jldsave(
        joinpath(out, "member_$(k).jld2");
        nee = surf["nee"],
        lhf = surf["lhf"],
        shf = surf["shf"],
        dates = collect(dates),
