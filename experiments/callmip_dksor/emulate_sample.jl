...
const OUTDIR =
    get(ENV, "CALLMIP_OUTDIR", joinpath(@__DIR__, "output_calibration"))
mkpath(OUTDIR)
...
if isfile(ces_train_path)
    ...
    isfile(ekp_path) || error(
        "Neither $ces_train_path nor $ekp_path found.\n" *
    ...
    emu_encoder_args = (;
        encoder_kwargs = Utilities.encoder_kwargs_from(
...
function prune_bad_columns(pairs::PairedDataContainer; threshold::Real = 1_000)
    ...
    isempty(bad_ext) ||
        @warn "  Pruning $(length(bad_ext)) extreme columns (|G| > $threshold)"
...
# ── Emulator validation ───────────────────────────────────────────────────────
y_pred_train, _ = Emulators.predict(
    emulator,
    get_inputs(train_pairs);
    transform_to_real = true,
)
y_pred_test, _ = Emulators.predict(
    emulator,
    get_inputs(test_pairs);
    transform_to_real = true,
)
...
try
    _, y_var_test = Emulators.predict(
        emulator,
        get_inputs(test_pairs);
        transform_to_real = true,
    )
    ...
    vars =
        y_var_test isa AbstractVector ?
        reduce(
            vcat,
            [
                vec(diag(v isa AbstractMatrix ? v : reshape([v], 1, 1))) for
                v in y_var_test
            ],
        ) : vec(y_var_test)
...
if get(ENV, "CALLMIP_CES_EKI_PRIOR", "true") == "true" && isfile(ekp_path)
    ...
    mcmc_prior, _ =
        build_dk_sor_eki_prior(vec(mean(ϕ; dims = 2)), vec(std(ϕ; dims = 2)))
...
posterior_samples_raw = reduce(
    vcat,
    [get_distribution(posterior)[name] for name in get_name(posterior)],
)
constrained_posterior =
    PD.transform_unconstrained_to_constrained(mcmc_prior, posterior_samples_raw)
eki_mean_constrained =
    PD.transform_unconstrained_to_constrained(mcmc_prior, init_sample)
...
# downstream UQ chain.
jldsave(
    joinpath(OUTDIR, "posterior_samples.jld2");
    constrained_posterior,
    param_names,
)
...
# ── Save posterior mean ───────────────────────────────────────────────────────
jldsave(
    joinpath(OUTDIR, "posterior_mean.jld2");
...
let
    ...
    prior_ens = PD.transform_unconstrained_to_constrained(
        prior,
        EKP.construct_initial_ensemble(Random.MersenneTwister(0), prior, 5000),
    )
    peaknorm(x; nb = 40) = begin
        lo, hi = minimum(x), maximum(x);
        hi <= lo && (hi = lo + eps(lo) + 1e-30)
        w = (hi - lo) / nb;
        ctr = [lo + (b - 0.5) * w for b in 1:nb];
        cnt = zeros(nb)
        for v in x
            ;
            cnt[clamp(Int(fld(v - lo, w)) + 1, 1, nb)] += 1;
        end
        m = maximum(cnt);
        m == 0 && (m = 1.0);
        (ctr, cnt ./ m)
    ...
    Legend(
        fig[nr + 1, 1:nc],
        [
            PolyElement(color = (:gray, 0.35)),
            PolyElement(color = (:steelblue, 0.55)),
