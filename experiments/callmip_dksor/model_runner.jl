...
const OUTDIR =
    joinpath(CLIMALAND_DIR, "experiments/callmip_dksor/output_prior_check")
...
const CHECK_YEARS =
    haskey(ENV, "CALLMIP_CHECK_YEARS") ?
...
"""
function run_prior_year(
    year;
    param_overrides::Dict = Dict{String, Float64}(),
    dt = DT,
    callmip::Bool = false,
    with_co2::Bool = true,
    return_sim::Bool = false,
    domain_override = nothing,
    free_drainage::Bool = false,
    soil_overrides = (;),
    rad_overrides = (;),
    T_init_override = nothing,
    met_path_override = nothing,
    spinup_days::Int = 0,
    stop_year::Int = year,
    init_state = nothing,
    ...
    for nm in ("α_NIR_leaf", "α_PAR_leaf", "ϵ_canopy")
        if haskey(param_overrides, nm)
            rad_overrides = merge(
                (; Symbol(nm) => pop!(param_overrides, nm)),
                rad_overrides,
            )
        end
    end
    ...
    if !isnothing(domain_override)
        ...
        haskey(domain_override, :nelements) &&
            (nelements = domain_override.nelements)
        haskey(domain_override, :dz_tuple) &&
            (dz_tuple = FT.(domain_override.dz_tuple))
    end
    land_domain = Column(;
    ...
    LAI, maxLAI = NCDataset(met_path, "r") do ds
        ...
        TimeVaryingInput(lai_secs[valid], lai_data[valid]),
        maximum(lai_data[valid])
    end
    ...
    haskey(soil_overrides, :soil_K_sat) &&
        (soil_K_sat = FT(soil_overrides.soil_K_sat))
    haskey(soil_overrides, :ν_ss_om) && (ν_ss_om = FT(soil_overrides.ν_ss_om))
    haskey(soil_overrides, :ν_ss_quartz) &&
        (ν_ss_quartz = FT(soil_overrides.ν_ss_quartz))
    haskey(soil_overrides, :ν_ss_gravel) &&
        (ν_ss_gravel = FT(soil_overrides.ν_ss_gravel))
    ...
    haskey(rad_overrides, :α_NIR_leaf) &&
        (α_NIR_leaf = FT(rad_overrides.α_NIR_leaf))
    haskey(rad_overrides, :τ_NIR_leaf) &&
        (τ_NIR_leaf = FT(rad_overrides.τ_NIR_leaf))
    haskey(rad_overrides, :α_PAR_leaf) &&
        (α_PAR_leaf = FT(rad_overrides.α_PAR_leaf))
    ...
    bottom_bc_kw =
        free_drainage ? (; bottom_bc = Soil.EnergyWaterFreeDrainage()) : (;)
    ...
    function set_ic!(Y, p, t, model)
        ...
        if isnothing(init_state)
            ...
            parent(Y.canopy.hydraulics.ϑ_l) .=
                parent(init_state.canopy.hydraulics.ϑ_l)
        end
    ...
    if callmip
        diags = vcat(
            ClimaLand.default_diagnostics(
                ...
                "";
                output_writer = ow,
                output_vars = CALLMIP_SURFACE_VARS,
            ...
            ClimaLand.default_diagnostics(
                ...
                "";
                output_writer = ow,
                output_vars = CALLMIP_COLUMN_VARS,
                reduction_period = :daily,
            ),
        )
    else
    ...
    if callmip
        ...
        for var in CALLMIP_SURFACE_VARS
            try
                ...
                isempty(ref_dates) &&
                    (ref_dates = Date.(date.(times)) .- Day(1))
            catch
                ...
                vec(
                    parent(
                        ClimaCore.Fields.coordinate_field(
                            axes(land.soil.domain.fields.z),
                        ).z,
                    ),
                ),
            )
...
function load_obs_year(year)
    NCDataset(FLUX_NC_PATH, "r") do ds
        ...
        for mon in 1:12
            mask =
                (Dates.year.(dates) .== year) .& (Dates.month.(dates) .== mon)
            nee_v = nee_all[mask];
            qle_v = qle_all[mask];
            qh_v = qh_all[mask]
            nee_f = filter(isfinite, nee_v);
            push!(nee_m, isempty(nee_f) ? NaN : mean(nee_f))
            lhf_f = filter(isfinite, qle_v);
            push!(lhf_m, isempty(lhf_f) ? NaN : mean(lhf_f))
            shf_f = filter(isfinite, qh_v);
            push!(shf_m, isempty(shf_f) ? NaN : mean(shf_f))
        end
        return nee_m, lhf_m, shf_m
    end
end
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    for yr in CHECK_YEARS
        ...
        try
            ...
            finite =
                ...
                all(isfinite, shf_m)
            @info @sprintf(
                "  %s year %d  NEE[%.2f,%.2f] LHF[%.1f,%.1f] SHF[%.1f,%.1f]",
                finite ? "OK  " : "BAD ",
                yr,
                minimum(nee_m),
                maximum(nee_m),
    ...
    for (row, (vk, ylabel, (rmse, r2))) in
        enumerate(zip(var_keys, var_labels, metrics))
