...
const SIM_OUTDIR =
    joinpath(CLIMALAND_DIR, "experiments/callmip_dksor/output_callmip_sims")
const NC_OUTDIR =
    joinpath(CLIMALAND_DIR, "experiments/callmip_dksor/output_netcdf")
...
function write_callmip_nc(
    ...
    time_days =
        Float64[1.0 + (dt - TIME_REF).value for dt in OUT_START:Day(1):OUT_STOP]
    ...
    for t in 1:N_DAYS
        ...
        (
            soilrn_[t] != FILL_VALUE &&
            soillhf_[t] != FILL_VALUE &&
            soilshf_[t] != FILL_VALUE
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    post_nc =
        joinpath(NC_OUTDIR, "ClimaLand.v1_Phase1a_DK-Sor_Cal_Posterior.nc")
