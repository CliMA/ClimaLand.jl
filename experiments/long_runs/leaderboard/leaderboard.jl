...
function compute_monthly_leaderboard(
    ...
    for (col, short_name) in enumerate(short_names)
        ...
        # Plot each year with the earlier year being more transparent than the latter years
        ...
        for (curr_year, (months, sim_vec, rmse_vec, bias_vec)) in enumerate(
            ...
            color = curr_year != num_years ? :blue : :black
    ...
    black_line = CairoMakie.LineElement(color = :black)
    CairoMakie.Legend(fig[1, 2], [black_line], ["Final year"])
...
function compute_seasonal_leaderboard(
    ...
    for short_name in short_names
        ...
        # Take time average
        sim_var_seasons = (
            !ClimaAnalysis.isempty(sim_var) ?
            ClimaAnalysis.average_time(sim_var) : sim_var for
            sim_var in sim_var_seasons
        )
        obs_var_seasons = (
            !ClimaAnalysis.isempty(obs_var) ?
            ClimaAnalysis.average_time(obs_var) : obs_var for
    ...
    for (col, short_name) in enumerate(short_names)
        ...
        for (curr_year, (seasons, sim_vec, rmse_vec, bias_vec)) in enumerate(
            ...
            curr_year != num_years ? color = :blue : color = :black
            alpha = curr_year / num_years
            data_vecs = [sim_vec, rmse_vec, bias_vec]
...
Partition `x_vec` and `y_vecs` into subarrays, so that the last value is of each
subarray is `val` if possible.
"""
function partition_by_val(val, x_vec, y_vecs...)
    for vec in y_vecs
        length(x_vec) != length(vec) && error(
            "Length of $x_vec ($(length(x_vec))) and length of $vec ($(length(y_vec))) are not the same",
        )
    end
