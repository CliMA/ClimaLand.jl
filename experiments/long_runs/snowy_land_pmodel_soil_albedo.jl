# # Global run of land model with experimental soil albedo parameterizations
...
#
# This variant constructs the land model component-by-component (instead of
# using the high-level `LandModel{FT}(forcing, LAI, toml_dict, domain, Δt; ...)`
# constructor) so that the soil model's albedo parameterization can be swapped
...
root_path = "snowy_land_pmodel_soil_albedo_longrun_$(device_suffix)"
...
@info "Run: Global Soil-Canopy-Snow Model (experimental soil albedo)"
