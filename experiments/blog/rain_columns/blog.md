...
***Where did the rain go?** The fate of the storm after thirty days. Bars to the right of zero are the four exits plus what is still in the soil. Bars to the left are water the column lost beyond the storm, drawn from what it held before the rain; the model does not tag individual raindrops, so the budget is a net accounting, and this bar appears only when the exits removed more water than the storm delivered. The grass spent 45 mm of pre-storm water on top of the storm; the forest spent 74 mm, of which about 26 mm came out of the trees' own tissue, which is what a forest's water potential dropping by two megapascals looks like in millimeters.*
...
The script below builds the bare-loam column and the forest column from the animation, runs them for thirty days, and plots three things: the bare column's water content through depth and time, both columns' water profiles on day 30, and the water each returns to the air. Both simulations together take under a minute on a laptop once the packages are compiled. It needs ClimaLand and CairoMakie installed, along with ClimaUtilities, ClimaDiagnostics, Insolation, and Thermodynamics, which come with ClimaLand.
...
***What the script produces.** Top right is the take-home in one panel: after a month the bare loam still holds the storm as a wet bulb between 30 cm and 1 m, to the right of the pre-storm line, because the sun can only dry the top few centimeters. Under the forest the whole profile sits to the left of that line: the roots have spent the storm and reached below it. The bottom panel shows the hourly rhythm behind the daily totals, with evaporation and transpiration peaking each afternoon. Change `loam` to sand or clay, the leaf area, or the rooting depth, and rerun.*
