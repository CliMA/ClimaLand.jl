...
The three textures send the same fifty millimeters to three different fates, and the animation above shows them side by side. **Sand passes it down.** The wetting front races downward and the surface is dry within hours; once the top few centimeters are dry they slow further evaporation to a trickle, and only 4 mm of the storm evaporates in a month. The front reaches the bottom of the meter within days, 27 mm has drained out toward groundwater by the end of the month, and the other 19 mm is still on its way down. **Loam sends half back to the sky and keeps the rest.** The front reaches about 40 cm on the first day and then spreads slowly deeper while the surface dries. Capillary flow keeps feeding the drying surface, so 27 mm of the storm evaporates, most of it in the first week; 2 mm drains, and the remaining 21 mm sits in the lower half of the column. **Clay refuses it.** With a saturated conductivity of about two millimeters per hour, the surface cannot admit rain at 4 mm per hour: 35 mm of the 50 runs off, 9 mm evaporates, 2 mm drains, and about 4 mm stays. The clay column also evaporates 24 mm of its own water over a month with no storm at all, because at the shared starting suction it is nearly saturated; the storm barely changes that.
...
We added an idealized forest to the loam column: five square meters of leaf per square meter of ground, a two-meter canopy, and roots spread through the whole meter. Everything else, the soil, the weather, and the starting state, is the same as in the bare loam column, so the difference between the two is the straw.

![Top: the storm's water evaporated plus transpired each day, for the four columns over 30 days, computed as the storm run minus the no-storm run. Bottom: daily minimum leaf water potential and daily mean moisture-stress factor for the forest.](figs/et_timeseries.png)

***Restricting the flow.** Top: the storm's water returned to the air each day in the four columns, computed as the storm run minus the no-storm run; runoff and drainage are not part of these curves. On the storm day itself the curves dip below zero, because the twelve humid, overcast hours evaporate less than the same clear day without rain. Bottom: for the forest, the daily minimum of the leaf water potential (solid) and the daily mean of the moisture-stress factor (dotted), the multiplier the model applies to photosynthesis and stomatal conductance, which falls to one half at a leaf potential of −2 MPa.*

The bare loam returns the storm's water quickly, most of its 27 mm in the first week while the surface is wet, and then goes quiet. The forest is on a different schedule. It transpires eight to nine millimeters a day whether or not it rained, so for the first four days the storm's water adds little: the roots would have taken as much from the soil's reserve. The storm's contribution peaks around day eight, at five to six millimeters a day, once the rain-free version of the column has run short, because roots reach water that surface evaporation cannot. That water is not free. By day ten the leaves reach a tension of two megapascals at midday, and the model's stress factor falls from 0.9 on day eight to 0.2 on day eleven. Transpiration falls to what the drying soil can supply, and photosynthesis with it. Even so, the forest keeps returning the storm's water to the air for weeks after the bare loam has gone quiet.

![Stacked horizontal bars showing, for each of four columns, how the storm's 50 mm was partitioned after 30 days into runoff, drainage, soil evaporation, transpiration, and water still in the soil.](figs/budget.png)
...
Under the forest the storm is almost gone within the month: 46 of the 50 mm went back to the air, 38 of them through the trees. The more striking number is the one the previous figure only hints at. With no rain at all, the forest column transpires 61 mm in a month, drawn from what the soil held before the storm. Vegetation does not wait for rain; it spends the sponge's reserve. Over the thirty days the forest returned 113 mm to the air, more than twice the storm, and finished the month with 36 mm less in its soil than it started with. This is the everyday physics behind two things that sound like paradoxes: that planting trees can dry the ground beneath them, and that a transpiring canopy stays cooler than dry bare ground on a summer afternoon while drying the soil faster.

## One model for the land, from a column to the globe

The four columns use two of ClimaLand's components: the soil model, which solves for water, ice, and heat, and the canopy model, which handles radiative transfer through the leaves, photosynthesis, stomatal conductance, plant hydraulics, and the canopy's own energy balance. ClimaLand also has a snow model, a soil carbon model with CO₂ diffusion through the pores, and a single-layer bucket model for fast experiments. Each component runs on its own, and they compose into integrated land models that exchange water, energy, and carbon at their interfaces, with the same conservation bookkeeping on one column or on a global cubed-sphere grid.
...
The complete experiment, with all four columns, the no-storm control, the water budget, and the animation, is in [rain_experiment.jl](rain_experiment.jl) and [plot_results.jl](plot_results.jl). The [ClimaLand documentation](https://clima.github.io/ClimaLand.jl/stable/) has tutorials that go from a single soil column to a global simulation driven by reanalysis data.

> **Notes on the setup**
>
> **Weather.** The forcing is idealized so that the four columns differ only in soil and vegetation: a clear-sky July at 38.7 °N with air at 19 to 31 °C, 50 percent relative humidity, a 2 m/s wind, and the sun's path computed for the date; the storm brings 95 percent humidity and overcast skies for its twelve hours. All columns start at a matric potential of −2 m (about −20 kPa) at every depth, with free drainage out of the bottom at 1 m. The no-storm control is the same month with the rain, the humidity spike, and the clouds removed.
>
