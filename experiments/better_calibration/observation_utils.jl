...
function diagnostics_lat_lon(nelements)
    ...
    domain = ClimaLand.Domains.SphericalShell(; radius = 0.1, depth = 0.1, nelements)
...
function make_ocean_mask(nelements)
    ...
    # TODO: This should not be expected for an user to write code like this
    attribs = Dict("short_name" => "landsea_mask", "long_name" => "Landsea mask")
    dim_attribs = Dict(
        "latitude" => Dict("units" => "degrees_north"),
        "longitude" => Dict("units" => "degrees_east"),
    )
    dims = Dict("latitude" => lats, "longitude" => longs)
    landsea_var = ClimaAnalysis.OutputVar(attribs, dims, dim_attribs, interpolated_mask)

    # For ERA5 data, it shouldn't matter what the threshold is since we already
    # resampled onto the diagnostics grid
    return ClimaAnalysis.generate_lonlat_mask(landsea_var, NaN, 1.0; threshold = 0.99)
end
