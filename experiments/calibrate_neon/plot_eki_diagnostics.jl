...
outputfolder = "output"
interation_folder = "iteration_005"

const SITE_ID = get(ENV, "NEON_SITE_ID", "NEON-moab")
const climaland_dir = pkgdir(ClimaLand)
timestring = "2025-01-01_2025-12-31"
const outputpath = get(ENV, "CALL_OUTPUT_PATH", "/kiwi-data/Data/groupMembers/evametz/ClimaLand_Output/Neon_calibration/NEON-moab/NEON-moab_$(timestring)/SpinUP-20d/CalDepth-0_02M/10-It/NEONextrapReal_newSO2_dt450s/")

const EKI_PATH = get(ENV, "CALL_EKI_PATH", joinpath(outputpath, outputfolder, interation_folder, "eki_file.jld2"))
const OBS_PATH  = joinpath(outputpath, "observations.jld2")
const OUTPUT_1  = get(ENV, "CALL_OUTPUT_1", joinpath(outputpath, outputfolder))
const OUTDIR    = joinpath(OUTPUT_1, "figures_eki_diagnostics/")
mkpath(OUTDIR)
...
ax1 = Axis(fig1[1, 1];
    xlabel = "Day index",
    ylabel = "Soil CO₂ (ppm)",
    title  = "$SITE_ID: Daily Soil CO₂ at ~$Caldepthnums — Prior vs Posterior vs Obs",
)
...
# ─────────────────────────────────────────────────────────────────────────────
#  PLOT 1a — Timeseries: daily soil CO₂ obs / posterior
# ─────────────────────────────────────────────────────────────────────────────
...
ax1 = Axis(fig1[1, 1];
    xlabel = "Day index",
    ylabel = "Soil CO₂ (ppm)",
    title  = "$SITE_ID: Daily Soil CO₂ at ~2 cm — Prior vs Posterior vs Obs",
)
...
ts_path = joinpath(OUTDIR, "plot_timeseries_post.png")
save(ts_path, fig1; px_per_unit=2)
println("  Saved → $ts_path")
