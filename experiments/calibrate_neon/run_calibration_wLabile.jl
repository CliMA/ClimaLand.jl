"""
ClimaCalibrate driver for NEON site soil CO₂ calibration with a depth-dependent
labile carbon / microbial-activity profile.

Calibrates 5 parameters (soilCO2_reference_rate, soilCO2_activation_energy,
michaelis_constant, O2_michaelis_constant, labile_depth_scale) against daily
soil CO₂ concentration observations at NEON sites using Unscented Kalman
Inversion.

labile_depth_scale (= k, 1/m) is the exponential decay rate of an extra depth
factor exp(k·z) folded into the prognostic SOC field (see
model_interface_wLabile.jl). It captures the decline of microbial activity with
depth beyond what the SOC abundance profile alone explains. We calibrate the
decay RATE (shape), not an amplitude, to avoid degeneracy with
soilCO2_reference_rate. k=0 recovers the constant-p_sx behaviour.
...
Usage:
    julia --project=.buildkite experiments/calibrate_neon/run_calibration_wLabile.jl
"""
...
output_base = joinpath(outputpath, "output_wLabile")
...
# - labile_depth_scale (k, 1/m): exp(k·z) decays with depth (z<0). NOT a
#   ClimaParams key — read directly from the TOML by model_interface_wLabile.jl.
#   lower bound 0 → exp(0·z)=1, recovering the constant-p_sx behaviour (null
#   anchor); mean 2.0 ≈ activity drops by ~factor e over ~0.5 m; upper bound
#   caps unphysically steep profiles.
...
labile_depth_scale        = [2.0, 1.0, 0.0, 10.0]
...
priors = [
    ...
    PD.constrained_gaussian(
        "labile_depth_scale",
        labile_depth_scale[1],
        labile_depth_scale[2],
        labile_depth_scale[3],
        labile_depth_scale[4],
    ),
]
...
open(PRIOR_VALUES_FILE, "w") do io
    ...
    println(io, "labile_depth_scale = $(labile_depth_scale)")
    ...
    println(io, "labile_depth_scale = $(labile_depth_scale[1])")
end
...
@everywhere include(    joinpath(pkgdir(ClimaLand), "experiments/calibrate_neon/model_interface_wLabile.jl"),
)

# ── Run Calibration ──────────────────────────────────────────────────────────

println("\n=== Starting ClimaCalibrate calibration (with labile depth profile) ===")
println("  Backend: WorkerBackend")
println("  Site: $SITE_ID")
println("  Iterations: $N_ITERATIONS")
println("  Ensemble size: $N_ens")
println("  Observation days: $n_obs")
println("  Parameters: $(length(priors)) (4 DAMM soilCO2 + labile_depth_scale)")
println("  LAI: MODIS")
