...
variant with the depth-dependent labile carbon / microbial-activity profile.

Adds the 5th calibrated parameter labile_depth_scale (= k, 1/m): an extra depth
factor exp(k·z) (z<0 → decays with depth) folded into the prognostic SOC field
in set_ic!, equivalent to a depth-dependent soluble-carbon fraction p_sx(z).
exp(k·0)=1 keeps the surface fixed; k=0 recovers the constant-p_sx behaviour.
...
Usage:
    julia --project=.buildkite experiments/calibrate_neon/run_prior_mean_wLabile.jl
"""
...
if PIPELINE_MODE
    ...
    output_base = joinpath(outputpath, "prior_runs/prior_mean_wLabile")
...
function set_prior()
    soilCO2_reference_rate = 3.7416e-7
    soilCO2_activation_energy = 57875.0
    michaelis_constant = 0.019087
    O2_michaelis_constant = 0.066995
    labile_depth_scale = 6.2047
    ...
    if isfile(Prior_filepath)
        ...
        for line in eachline(Prior_filepath)
            ...
            elseif key == "labile_depth_scale"
                labile_depth_scale = val
            end
        end
        ...
        println("  labile_depth_scale        = $labile_depth_scale")
    else
        println("Prior mean parameters file not found at $Prior_filepath, using default values.")
    end
    return soilCO2_reference_rate, soilCO2_activation_energy, michaelis_constant, O2_michaelis_constant, labile_depth_scale
end
soilCO2_reference_rate, soilCO2_activation_energy, michaelis_constant, O2_michaelis_constant, labile_depth_scale = set_prior()
println("  soilCO2_reference_rate    = $soilCO2_reference_rate")
println("  soilCO2_activation_energy = $soilCO2_activation_energy")
println("  michaelis_constant        = $michaelis_constant")
println("  O2_michaelis_constant     = $O2_michaelis_constant")
...
function set_ic!(Y, p, t, model)
    ...
    # Capture labile_depth_scale into a concrete-typed local first: read as a
    # global inside the closure it infers as `Any`, which makes the map result
    # type-unstable and breaks ClimaCore's broadcast (eltype_error).
