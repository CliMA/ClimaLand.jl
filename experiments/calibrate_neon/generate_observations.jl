...
Reads NEON soil CO₂ concentration observations at depth 501 (~2 cm),
...
obs_df[!, :sco2_var_501] =
    [rowvar_skipinvalid(row, co2_cols_501) for row in eachrow(obs_df)]
...
# ── Noise covariance ────────────────────────────────────────────────────────
mean_sensor_var = mean(filter(!isnan, daily_df.daily_var)) #/ 2
noise_diag = fill(mean_sensor_var, n_obs)
#noise_diag = [isnan(v) ? mean_sensor_var : v for v in daily_df.daily_var]
...
mkpath(outputpath)
obs_filepath =
    joinpath(outputpath, "observations.jld2")
JLD2.jldsave(
    obs_filepath;
