...
Extends model_interface.jl by adding a 4th calibrated parameter:
nu_scaling_factor — a scalar multiplier applied to the spatially-varying
soil porosity field (ν) after LandModel construction.

Because nu_scaling_factor is not a ClimaParams TOML key, it is read
directly via Julia's built-in TOML parser rather than LP.create_toml_dict.
The porosity field is then scaled in-place:
    land.soil.parameters.ν .*= nu_scale   (clamped to [0.05, 0.95])
...
function ClimaCalibrate.forward_model(iteration, member)
    ...
    # Load calibrated TOML path
    calibrate_params_path =
        ClimaCalibrate.parameter_path(OUTPUT_DIR, iteration, member)

    # Read ClimaParams-registered parameters via LP (soilCO2, michaelis, O2_michaelis)
    toml_dict =
        LP.create_toml_dict(FT; override_files = [calibrate_params_path])

    # Read nu_scaling_factor directly — NOT a ClimaParams key, so LP ignores it
    calib_params_raw = TOML_pkg.parsefile(calibrate_params_path)
    nu_scale = FT(calib_params_raw["nu_scaling_factor"]["value"])
    @info "Member $member: nu_scaling_factor = $nu_scale"
    ...
    # ── Apply porosity scaling ────────────────────────────────────────────────
    # land.soil.parameters.ν is a ClimaCore Field (mutable even inside
    # immutable structs). Scale in-place and clamp to physical bounds.
    @. land.soil.parameters.ν = clamp(
        land.soil.parameters.ν * nu_scale,
        FT(0.05),
        FT(0.95),
    )
    ...
    function custom_set_ic!(Y, p, t, model)
        ...
        # Load SOC profile from NEON-derived CSV
        data = CSV.read(
            "/kiwi-data/Data/groupMembers/evametz/Neon/Neon_data/NEON_all_sites_estimatedOC_2cm_mean_extrapolated.csv",
            DataFrame,
        )
